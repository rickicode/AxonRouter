...
AxonRouter is a local AI routing gateway and dashboard built on Next.js.
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.axonrouter/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.axonrouter/usage.json`, `~/.axonrouter/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.axonrouter/db.json`)
- usage stats: `~/.axonrouter/usage.json`
- request log lines: `~/.axonrouter/log.txt`
...
- Build from source: `cd /root/dev/axonrouter && npm run build`
