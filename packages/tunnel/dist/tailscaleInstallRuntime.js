...
async function installTailscaleMac(sudoPassword, log) {
        ...
        await new Promise((resolve, reject) => {
            ...
            child.on("close", (c) => {
                if (c === 0)
                    ...
                    reject(new Error(`brew install failed (code ${c})`));
            });
    ...
    await new Promise((resolve, reject) => {
        ...
        child.on("close", (c) => {
            try {
                execSyncCmd(`rm -f ${pkgPath}`, { stdio: "ignore", windowsHide: true });
            }
            catch {
                // ignore
            }
...
async function installTailscaleLinux(sudoPassword, log) {
    ...
    return new Promise((resolve, reject) => {
        ...
        curlChild.on("exit", (code) => {
            if (code !== 0)
                return reject(new Error(`Failed to download install script: ${curlErr}`));
            log("Running install script...");
            const child = spawnCmd("sudo", ["-S", "sh"], { stdio: ["pipe", "pipe", "pipe"], windowsHide: true });
            let stderr = "";
            child.stdout.on("data", (d) => {
                const line = d.toString().trim();
                if (line)
...
async function installTailscaleWindows(log) {
    ...
    while (Date.now() - start < maxWait) {
        if (dataFileExists(WINDOWS_TAILSCALE_BIN)) {
...
export async function installTailscaleRuntime(sudoPassword, hostname, onProgress) {
    ...
    if (IS_MAC)
        await installTailscaleMac(sudoPassword, log);
    else
        await installTailscaleLinux(sudoPassword, log);
    log("Starting daemon...");
