...
function resolveTailscaleBin() {
    try {
        const systemPath = execSyncCmd("which tailscale 2>/dev/null || where tailscale 2>nul", { encoding: "utf8", windowsHide: true }).trim();
        if (systemPath)
            return systemPath;
    }
    catch {
...
function getTailscaleBin() {
    if (cachedTailscaleBin !== undefined)
...
function parseFunnelUrl(text) {
    return (text.match(FUNNEL_URL_REGEX) || [])[0]?.replace(/\/$/, "") || null;
}
export async function startFunnelRuntime(port) {
    const bin = getTailscaleBin();
    if (!bin)
        throw new Error("Tailscale not installed");
    const socketArgs = getTailscaleSocketArgs();
    try {
        execSyncCmd(`"${bin}" ${socketArgs.join(" ")} funnel --bg reset`, { stdio: "ignore", windowsHide: true });
    }
    catch {
        // ignore
    }
    return new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(async () => {
            ...
            if (url)
                ...
                reject(new Error(`Tailscale funnel timed out: ${output.trim() || "no output"}`));
        }, 30000);
        ...
        child.on("exit", async (code) => {
            ...
            if (url)
                ...
                reject(new Error(`tailscale funnel failed (code ${code}): ${output.trim()}`));
        });
        child.on("error", (err) => {
            if (resolved)
                return;
...
export async function stopDaemonRuntime(sudoPassword) {
    try {
        execSyncCmd("pkill -x tailscaled", { stdio: "ignore", windowsHide: true, timeout: 3000 });
    }
    catch {
        // ignore
    }
    ...
    if (!IS_WINDOWS) {
        ...
        try {
            if (dataFileExists(socketPath))
                unlinkDataFile(socketPath);
        }
        catch {
            // ignore
        }
