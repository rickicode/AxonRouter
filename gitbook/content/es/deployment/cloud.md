...
Despliega AxonRouter en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.axonrouter` | Ruta de almacenamiento de la base de datos |
...
```bash
sudo mkdir -p /var/lib/axonrouter
sudo chown $USER:$USER /var/lib/axonrouter
```
...
Crea `/etc/nginx/sites-available/axonrouter`:
...
sudo ln -s /etc/nginx/sites-available/axonrouter /etc/nginx/sites-enabled/
...
# Si NO usas proxy reverso, permite los puertos de AxonRouter
...
# Actualizar AxonRouter
cd /path/to/axonrouter/app
git pull
npm install
npm run build
pm2 restart axonrouter
...
tar -czf axonrouter-backup-$(date +%Y%m%d).tar.gz /var/lib/axonrouter

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/axonrouter-$(date +\%Y\%m\%d).tar.gz /var/lib/axonrouter
```
...
```bash
# Verificar si AxonRouter está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/axonrouter
chmod 755 /var/lib/axonrouter
```
