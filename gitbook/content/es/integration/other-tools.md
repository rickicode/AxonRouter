...
AxonRouter es compatible con cualquier herramienta que soporte el formato de API de OpenAI. Esta guía cubre patrones de integración genéricos para varias herramientas y aplicaciones personalizadas.

## Resumen

AxonRouter proporciona un endpoint de API compatible con OpenAI que funciona con:
...
Cualquier herramienta compatible con OpenAI puede conectarse a AxonRouter usando estas configuraciones:

**AxonRouter local:**
```
Base URL: http://localhost:3778/v1
API Key: your-api-key-from-dashboard
Model: cualquier modelo de AxonRouter (cc/*, cx/*, glm/*, etc.)
```

**AxonRouter en la nube:**
```
Base URL: https://axonrouter.com/v1
API Key: your-api-key-from-dashboard
Model: cualquier modelo de AxonRouter (cc/*, cx/*, glm/*, etc.)
```
...
client = OpenAI(
    ...
    base_url="http://localhost:3778/v1"
)
...
const client = new OpenAI({
  ...
  baseURL: "http://localhost:3778/v1"
});
...
**Solicitud:**
```
POST http://localhost:3778/v1/chat/completions
```
...
llm = ChatOpenAI(
    ...
    openai_api_base="http://localhost:3778/v1",
    temperature=0.7
)
...
llm = OpenAI(
    model="cc/claude-sonnet-4-20250514",
    api_key="your-api-key-from-dashboard",
    api_base="http://localhost:3778/v1"
)
...
openai.api_base = "http://localhost:3778/v1"
...
ROUTER_BASE_URL=http://localhost:3778/v1
...
**Problema:** No se puede conectar a AxonRouter
```bash
# Verifica si AxonRouter está corriendo
...
**Solución:**
- Verifica que AxonRouter esté corriendo
- Verifica que el puerto 3778 no esté bloqueado
...
- Verifica los modelos disponibles: `curl http://localhost:3778/v1/models`
...
- Verifica la conexión de red a AxonRouter
