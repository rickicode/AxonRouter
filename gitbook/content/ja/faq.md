...
AxonRouterに関する一般的な質問。

---

## AxonRouterとは?

**AxonRouterは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**はい、AxonRouter自体は100%無料でオープンソースです。**
...
**AxonRouterはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
## AxonRouterをセルフホストできますか?

**はい! AxonRouterは複数のデプロイメントオプションをサポートします:**
...
- `DATA_DIR` - データベース保存パス(デフォルト: `~/.axonrouter`)
...
**はい、AxonRouterはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.axonrouter`(またはカスタム`DATA_DIR`)にローカル保存
- AxonRouterサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
