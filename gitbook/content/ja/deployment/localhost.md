...
開発と個人利用のため、ローカルマシンでAxonRouterを実行。

---

## 📦 インストール

npm経由でAxonRouterをグローバルインストール:
...
- **APIエンドポイント**: `http://localhost:3778/v1`
- **データディレクトリ**: `~/.axonrouter`
...
APIポート(3778)とダッシュボードポート(3000)はアプリケーションで設定されています。変更するにはソースコードを修正するか、サポートされている場合は環境変数を使用してください。

---

## 🛑 サーバーの停止

AxonRouterが実行されているターミナルで `Ctrl+C` を押します。

```bash
# axonrouterを実行しているターミナル
^C  # Ctrl+Cを押す
```
