...
AxonRouterをClaude Code CLIと統合し、AnthropicのAPIリクエストをAxonRouterのインテリジェントルーティングシステム経由でルーティングします。
...
Claude CodeはAxonRouterモデルにマッピングされる以下のモデルエイリアスをサポート:
...
1. モデル名がAxonRouterの設定と一致しているか確認
2. AxonRouterダッシュボードでプロバイダー接続がアクティブか確認
3. 接続されたプロバイダーでモデルが利用可能か確認

## クラウドエンドポイント

localhostの代わりにAxonRouterクラウドエンドポイントを使用するには:

```bash
export ANTHROPIC_BASE_URL="https://axonrouter.com"
```

AxonRouterクラウドダッシュボードでAPIキーが設定されていることを確認してください。
