...
Tích hợp AxonRouter với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của AxonRouter.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- AxonRouter đang chạy cục bộ hoặc cloud endpoint đã cấu hình
...
Đặt base URL tới endpoint AxonRouter:
...
**Cho AxonRouter cloud:**
```
https://axonrouter.com
```

**Các bước:**
1. Trong field **Base URL**, nhập endpoint AxonRouter
2. Đảm bảo bao gồm `/v1` ở cuối

### 4. Thêm API Key

1. Trong field **API Key**, nhập API key AxonRouter của bạn
2. Bạn có thể tìm API key trong AxonRouter dashboard tại **Settings → API Keys**
3. Key bắt đầu bằng `sk-axonrouter-`

### 5. Chọn Model

1. Trong dropdown **Model**, bạn có thể:
   - Chọn từ model có sẵn (nếu Cline auto-detect)
   - Nhập tên model thủ công từ cấu hình AxonRouter
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng AxonRouter để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua AxonRouter
...
3. Nhận giải thích AI qua AxonRouter

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng AxonRouter để hiểu context và thực hiện thay đổi
...
1. Xác minh AxonRouter đang chạy: `curl http://localhost:3778/health`
2. Kiểm tra base URL đúng và bao gồm `/v1`
3. Đảm bảo không firewall nào chặn port 3778
...
2. Xác minh AxonRouter instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs AxonRouter để xem lỗi

## Cấu hình Nâng cao

### Dùng Cloud Endpoint

Để dùng AxonRouter cloud endpoint thay vì localhost:

1. Trong Cline settings, đặt Base URL: `https://axonrouter.com`
2. Đảm bảo bạn đã cấu hình API key trong AxonRouter cloud dashboard
...
## Tích hợp với Tính năng AxonRouter

### Định tuyến Model

AxonRouter tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, AxonRouter tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua AxonRouter dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
