...
**AxonRouter 是一款 AI 模型路由工具,能够最大化你的订阅价值并最小化成本。**
...
**环境变量:**
- `JWT_SECRET` - **生产环境必须修改!**
- `DATA_DIR` - 数据库存储路径(默认:`~/.axonrouter`)
...
**是的,AxonRouter 优先考虑安全和隐私:**

**本地存储:**
- 所有数据存储在本地 `~/.axonrouter`(或自定义 `DATA_DIR`)
- 不会发送数据到 AxonRouter 服务器
