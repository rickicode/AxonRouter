...
export function openaiToGeminiCLIRequest(model, body, stream) {
  ...
  // Enforce strict JSON-only output when client requests OpenAI JSON mode.
  // Gemini often wraps JSON in markdown fences unless explicitly constrained.
  if (body.response_format?.type === "json_object") {
    const jsonOnlyInstruction = "Return ONLY a valid JSON object. Do not use markdown fences, prose, or extra keys.";
    if (gemini.systemInstruction?.parts?.length) {
      gemini.systemInstruction.parts.push({ text: jsonOnlyInstruction });
    } else {
      gemini.systemInstruction = {
        role: "user",
        parts: [{ text: jsonOnlyInstruction }],
      };
    }

    // Prevent premature empty completions on strict JSON mode.
    const currentMax = Number(gemini.generationConfig?.maxOutputTokens || 0);
    if (!Number.isFinite(currentMax) || currentMax < 256) {
      gemini.generationConfig.maxOutputTokens = 256;
    }
  }
