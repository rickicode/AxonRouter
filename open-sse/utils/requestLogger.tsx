...
// Logging is enabled via global flag set by app initialization from DB settings
function isLoggingEnabled() {
  return (globalThis as any).__AXONROUTER_REQUEST_LOGS_ENABLED === true;
}
...
async function createLogSession(sourceFormat, targetFormat, model) {
  ...
  try {
    ...
    const safeModel = (model || "unknown").replace(/[^a-zA-Z0-9._-]/g, "-");
    const folderName = `${sourceFormat}_${targetFormat}_${safeModel}_${timestamp}`;
    const sessionPath = pathMod.join(LOGS_DIR, folderName);
...
function writeJsonFile(sessionPath, filename, data) {
  ...
  try {
    const filePath = pathMod.join(sessionPath, filename);
...
export async function createRequestLogger(sourceFormat, targetFormat, model) {
  ...
  return {
    ...
    appendProviderChunk(chunk) {
      if (!fs || !sessionPath) return;
      try {
        const filePath = pathMod.join(sessionPath, "5_res_provider.txt");
    ...
    appendOpenAIChunk(chunk) {
      if (!fs || !sessionPath) return;
      try {
        const filePath = pathMod.join(sessionPath, "6_res_openai.txt");
        fs.appendFileSync(filePath, chunk);
      } catch (err) {
        // Ignore append errors
      }
    ...
    appendConvertedChunk(chunk) {
      if (!fs || !sessionPath) return;
      try {
        const filePath = pathMod.join(sessionPath, "7_res_client.txt");
        fs.appendFileSync(filePath, chunk);
      } catch (err) {
        // Ignore append errors
      }
...
export function logError(provider, { error, url, model, requestBody }) {
  ...
  try {
    ...
    const logPath = pathMod.join(LOGS_DIR, `${provider}-${date}.log`);
