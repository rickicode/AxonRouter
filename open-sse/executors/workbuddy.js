 ...
 *
 * Same OpenAI-compatible-but-stream-only gateway as CodeBuddy Intl (same Tencent
 * "Buddy AI" backend, different host/brand). Non-stream requests are rejected and
 * reasoning is surfaced only when the request carries the IDE's OpenAI-style
 * reasoning params. Force stream and mirror reasoning_summary exactly like the
 * CodeBuddy executors.
 */
export class WorkBuddyExecutor extends DefaultExecutor {
  ...
  transformRequest(model, body, stream, credentials) {
    ...
    // WorkBuddy rejects plain OpenAI shape (11101 invalid request): needs a
    ...
    transformed.messages = [{ role: "system", content: "You are WorkBuddy." }];
