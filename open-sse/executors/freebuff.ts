...
export class FreebuffExecutor extends BaseExecutor {
  ...
  async execute(args: any) {
    const { credentials, body, stream: clientWantsStream, signal, log, proxyOptions = null } = args;
    const token = credentials.apiKey || credentials.accessToken;
    ...
    if (!session.active) {
      ...
      const detail = activeSessionPayload?.message
        || interpretation
    ...
    if (!isFreebuffSessionActive(activeSessionPayload)) {
      throw new Error("Freebuff free session is not active. Start/restart the freebuff CLI and wait for admission.");
    }
    ...
    const clientId = extractFreebuffFingerprint(activeSessionPayload) || resolveFreebuffClientId(credentials);
    const freebuffInstanceId = extractFreebuffFingerprint(activeSessionPayload) || clientId || FREEBUFF_DEFAULT_CLIENT_ID;
    ...
    const enhancedBody = this.buildEnhancedBody(body, rawModel, runId, clientId, freebuffInstanceId);
    ...
    const shouldRetry = result.response.status === 426 || result.response.status === 409;
    if (shouldRetry) {
      log?.debug?.("FREEBUFF", `Got ${result.response.status}, force-joining fresh session and retrying`);
      try {
        return await this.retryWithFreshSession(args, rawModel, token, signal, proxyOptions, log);
      } catch (retryError: any) {
        log?.error?.("FREEBUFF", `Retry after 426 failed: ${retryError.message}`);
  ...
  private async retryWithFreshSession(
    ...
    const freshClientId = extractFreebuffFingerprint(freshPayload) || resolveFreebuffClientId(credentials);
    const freshInstanceId = extractFreebuffFingerprint(freshPayload) || freshClientId || FREEBUFF_DEFAULT_CLIENT_ID;
    ...
    const freshBody = this.buildEnhancedBody(body, rawModel, freshRunId, freshClientId, freshInstanceId);

    log?.info?.("FREEBUFF", `Retry with fresh session: instance=${freshInstanceId.slice(0, 8)}...`);
