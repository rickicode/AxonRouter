...
import { OAUTH_ENDPOINTS, ANTIGRAVITY_HEADERS, INTERNAL_REQUEST_HEADER, AG_DEFAULT_TOOLS, AG_TOOL_SUFFIX } from "../config/appConstants";
...
export class AntigravityExecutor extends BaseExecutor {
  ...
  buildUrl(model: any, stream: any, urlIndex = 0) {
  ...
  buildHeaders(credentials: any, stream = true, sessionId: any = null) {
    return {
  ...
  async execute({ model, body, stream, credentials, signal, log, proxyOptions = null }) {
    ...
    for (let urlIndex = 0; urlIndex < fallbackCount; urlIndex++) {
      ...
      try {
        const response: any = await proxyAwareFetch(url, {
          method: "POST",
          headers,
