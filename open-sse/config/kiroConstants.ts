 ...
 *   - `KIRO_AGENTIC_SYSTEM_PROMPT` chunked-write prompt for `-agentic`
 *     synthetic variants
 *
 * Kiro upstream does not advertise `-agentic` or `-thinking` model IDs;
 * those are synthetic aliases that this layer resolves before sending
 ...
 */
export function isThinkingEnabled(body: any, headers?: any, model?: string): boolean {
  if (headers) {
...
/**
 * Detect whether a model id refers to a synthetic agentic variant.
 ...
 */
export function stripAgenticSuffix(model: string): string {
  if (!isAgenticModel(model)) return model;
  return model.slice(0, -KIRO_AGENTIC_SUFFIX.length);
}

/**
 * Detect whether a model id is a synthetic thinking variant
...
/**
 * Resolve a model id to the real upstream Kiro model id, plus flags
 ...
 */
export function resolveKiroModel(model: string): { upstream: string; agentic: boolean; thinking: boolean } {
  let upstream = model;
  let agentic = false;
  let thinking = false;
...
function pickHeader(headers: any, name: string): string | undefined {
...
function containsThinkingModeTag(body: any): boolean {
