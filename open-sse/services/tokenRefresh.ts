...
function getRefreshFlightKey(provider, credentials) {
  const refreshToken = credentials?.refreshToken || "";
  const accessToken = credentials?.accessToken || "";
  return `${provider}:${refreshToken || accessToken}`;
}

async function runRefreshSingleFlight(provider, credentials, task) {
  const key = getRefreshFlightKey(provider, credentials);
  if (!key || key.endsWith(":")) return task();

  const existing = tokenRefreshFlights.get(key);
  if (existing) return existing;

  const promise = Promise.resolve()
    .then(task)
    .finally(() => {
      if (tokenRefreshFlights.get(key) === promise) tokenRefreshFlights.delete(key);
    });
  tokenRefreshFlights.set(key, promise);
  return promise;
}

export function __resetTokenRefreshSingleFlightForTests() {
...
function parseKiroRefreshFailure(status, errorText, source, log) {
  let errorCode = null;
  try {
    const parsed = JSON.parse(errorText);
    errorCode =
  ...
  const normalizedCode = typeof errorCode === "string" ? errorCode.toLowerCase() : null;
  const unrecoverable =
    status === 400 ||
    status === 401 ||
    status === 403 ||
  ...
  log?.error?.("TOKEN_REFRESH", `Failed to refresh ${source}`, {
    status,
    error: errorText,
  ...
  if (unrecoverable) {
    return {
      ...
      code: normalizedCode || "invalid_grant",
    };
  }
...
export async function refreshKiroToken(refreshToken, providerSpecificData, log) {
  ...
  try {
    ...
    if (clientId && clientSecret) {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        return parseKiroRefreshFailure(response.status, errorText, "Kiro AWS token", log);
      }
    ...
    if (!response.ok) {
      const errorText = await response.text();
      return parseKiroRefreshFailure(response.status, errorText, "Kiro social token", log);
    }
...
export async function refreshGitHubToken(refreshToken, log) {
  ...
  const githubProvider: any = PROVIDERS.github;
  if (githubProvider.clientSecret) {
    params.client_secret = githubProvider.clientSecret;
  }
...
export async function getAccessToken(provider, credentials, log) {
  ...
  return runRefreshSingleFlight(provider, credentials, async () => {
    switch (provider) {
      case "gemini":
      case "gemini-cli":
      case "antigravity":
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    try {
      ...
      if (isUnrecoverableRefreshError(result)) {
        log?.error?.("TOKEN_REFRESH", "Unrecoverable refresh error detected; skipping retries");
        return result;
      }
