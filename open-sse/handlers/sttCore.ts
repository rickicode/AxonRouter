...
async function upstreamError(res) {
  ...
  return createErrorResult(res.status, typeof msg === "string" ? msg : JSON.stringify(msg), null);
}
...
async function transcribeAssemblyAI(cfg, file, model, token) {
  ...
  while (Date.now() - start < 120000) {
    ...
    if (r.status === "error") return createErrorResult(500, r.error || "AssemblyAI failed", null);
  }
  return createErrorResult(504, "AssemblyAI timeout after 120s", null);
}
...
async function transcribeHuggingFace(cfg, file, model, token) {
  if (model.includes("..") || model.includes("//")) return createErrorResult(400, "Invalid model ID", null);
...
export async function handleSttCore({ provider, model, formData, credentials }) {
  ...
  if (!file) return createErrorResult(HTTP_STATUS.BAD_REQUEST, "Missing required field: file", null);

  const cfg = AI_PROVIDERS[provider]?.sttConfig;
  if (!cfg) return createErrorResult(HTTP_STATUS.BAD_REQUEST, `Provider '${provider}' does not support STT`, null);

  const token = cfg.authType === "none" ? null : (credentials?.apiKey || credentials?.accessToken);
  if (cfg.authType !== "none" && !token) {
    return createErrorResult(HTTP_STATUS.UNAUTHORIZED, `No credentials for STT provider: ${provider}`, null);
  }
  ...
  try {
    ...
    return createErrorResult(HTTP_STATUS.BAD_GATEWAY, err.message || "STT request failed", null);
  }
}
