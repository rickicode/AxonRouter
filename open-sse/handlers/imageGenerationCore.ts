...
export async function handleImageGenerationCore({
  ...
  if (!body.prompt) {
    return createErrorResult(HTTP_STATUS.BAD_REQUEST, "Missing required field: prompt", null);
  }
  ...
  try {
    ...
    return createErrorResult(HTTP_STATUS.BAD_GATEWAY, `Invalid response from ${provider}`, null);
  }
