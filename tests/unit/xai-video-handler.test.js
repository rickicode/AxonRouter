 ...
 *  - connection id surfaced via x-axonrouter-connection-id
...
describe("handleVideoCreate", () => {
  ...
  it("returns the serving connection id in x-axonrouter-connection-id", async () => {
    ...
    expect(res.headers.get("x-axonrouter-connection-id")).toBe("conn-77");
  ...
  it("rotates to the next account on 401 (auth errors cannot have created a job)", async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.headers.get("x-axonrouter-connection-id")).toBe("conn-2");
    expect(authMocks.markAccountUnavailable).toHaveBeenCalledWith(
      "conn-1", 401, expect.any(String), "xai", null, undefined
    );
  });
