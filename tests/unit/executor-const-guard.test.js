...
import kilocodeFree from "../../open-sse/providers/registry/kilocode-free.js";
...
describe("provider baseUrl const (full path, no trailing slash)", () => {
  it("kilocode-free full path", () => {
    expect(kilocodeFree.transport.baseUrl).toBe("https://api.kilo.ai/api/gateway/chat/completions");
  });
...
describe("antigravity quota retry policy", () => {
  it("429 is delegated to account fallback without executor retry", () => {
    expect(antigravity.transport.retry["429"].attempts).toBe(0);
  });
  it("503 is delegated to account/model fallback without retry", () => {
