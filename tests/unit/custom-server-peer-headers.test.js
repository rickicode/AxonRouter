...
describe("custom-server peer header sanitizing", () => {
  ...
  it("replaces a client-supplied x-axonrouter-real-ip with the socket address", async () => {
    const headers = await get({ "x-axonrouter-real-ip": "203.0.113.55" });

    expect(headers["x-axonrouter-real-ip"]).toMatch(/^(::ffff:)?127\.0\.0\.1$/);
  });

  it("stamps the trust token so downstream can tell the wrapper ran", async () => {
    const headers = await get();

    expect(headers["x-axonrouter-peer-token"]).toBe(process.env.AXONROUTER_PEER_TOKEN);
  });

  it("drops a client-supplied peer trust token", async () => {
    const headers = await get({ "x-axonrouter-peer-token": "forged-token" });

    expect(headers["x-axonrouter-peer-token"]).toBe(process.env.AXONROUTER_PEER_TOKEN);
    expect(headers["x-axonrouter-peer-token"]).not.toBe("forged-token");
  });

  it("drops a client-supplied x-axonrouter-via-proxy marker", async () => {
    const headers = await get({ "x-axonrouter-via-proxy": "1" });

    expect(headers["x-axonrouter-via-proxy"]).toBeUndefined();
  });
  ...
  it("marks via-proxy and adopts the forwarded IP for a loopback proxy hop", async () => {
    ...
    expect(headers["x-axonrouter-via-proxy"]).toBe("1");
    expect(headers["x-axonrouter-real-ip"]).toBe("203.0.113.9");
  ...
  it("keeps the peer token out of persisted request details", () => {
    const sanitized = requestDetails.sanitizeHeaders({
      "x-axonrouter-peer-token": "secret",
      "x-axonrouter-cli-token": "secret",
      "authorization": "Bearer sk-x",
      "x-axonrouter-real-ip": "127.0.0.1",
    });

    expect(sanitized["x-axonrouter-peer-token"]).toBeUndefined();
    expect(sanitized["x-axonrouter-cli-token"]).toBeUndefined();
    expect(sanitized["authorization"]).toBeUndefined();
    expect(sanitized["x-axonrouter-real-ip"]).toBe("127.0.0.1");
  });
});
