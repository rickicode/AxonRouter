...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("axonrouter");
    expect(parsed.model).toMatchObject({
      model: "cx/gpt-5.6-sol",
      base_url: "http://127.0.0.1:10128/v1",
      context_window: 400000,
    });
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "axonrouter-general-purpose",
      explore: "axonrouter-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.axonrouter\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.axonrouter-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.axonrouter-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# axonrouter-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.axonrouter-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("axonrouter-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.axonrouter-");
    expect(reset).not.toContain("axonrouter-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("axonrouter-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("axonrouter-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("axonrouter-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("axonrouter-explore");
    expect(getGrokSubagentSlot("plan")).toBe("axonrouter-plan");
