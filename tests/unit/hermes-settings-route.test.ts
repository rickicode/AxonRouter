...
describe("hermes settings route", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
    execAsyncMock.mockResolvedValue({ stdout: "/usr/local/bin/hermes\n", stderr: "" });
    readFileMock.mockResolvedValue("");
  });

  it("normalizes supported AxonRouter model shortcuts before writing Hermes config", async () => {
    const { POST } = await import("../../src/app/api/cli-tools/hermes-settings/route");
    ...
    expect(writeFileMock).toHaveBeenCalledWith(
      ...
      expect.stringContaining('default: "cx/gpt-5.3-codex"'),
    );
  });
