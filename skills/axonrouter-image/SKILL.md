---
name: axonrouter-image
description: Generate images via AxonRouter /v1/images/generations using OpenAI / Gemini Imagen / DALL-E / FLUX / MiniMax / SDWebUI / ComfyUI / Codex models. Use when the user wants to create, generate, draw, or render an image, picture, or text-to-image (txt2img).
---

# AxonRouter — Image Generation
...
```bash
curl $AXONROUTER_URL/v1/models/image | jq '.data[].id'
# Per-model params/options (size enum, quality enum, capabilities like edit)
curl "$AXONROUTER_URL/v1/models/info?id=openai/dall-e-3"
```

## Endpoint

`POST $AXONROUTER_URL/v1/images/generations`
...
```bash
curl -X POST "$AXONROUTER_URL/v1/images/generations?response_format=binary" \
...
```js
const r = await fetch(`${process.env.AXONROUTER_URL}/v1/images/generations`, {
  method: "POST",
...
result = client.images.generate(model="openai/dall-e-3", prompt="neon city at night", size="1024x1024")
