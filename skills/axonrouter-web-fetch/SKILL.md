---
name: axonrouter-web-fetch
description: Fetch URL → markdown / text / HTML via AxonRouter /v1/web/fetch using Firecrawl / Jina Reader / Tavily Extract / Exa Contents. Use when the user wants to scrape a webpage, extract URL content, read article, or convert a URL to markdown.
---

# AxonRouter — Web Fetch
...
```bash
curl $AXONROUTER_URL/v1/models/web | jq '.data[] | select(.kind=="webFetch") | .id'
# Per-provider params
curl "$AXONROUTER_URL/v1/models/info?id=firecrawl/fetch"
```
...
`POST $AXONROUTER_URL/v1/web/fetch`
...
### Jina Reader
```bash
curl -X POST $AXONROUTER_URL/v1/web/fetch \
  -H "Authorization: Bearer $AXONROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"jina-reader","url":"https://axonrouter.com","format":"markdown"}'
```
...
```js
const r = await fetch(`${process.env.AXONROUTER_URL}/v1/web/fetch`, {
  method: "POST",
...
r = requests.post(f"{os.environ['AXONROUTER_URL']}/v1/web/fetch",
    headers={"Authorization": f"Bearer {os.environ['AXONROUTER_KEY']}"},
    json={"model": "jina-reader", "url": "https://example.com", "format": "markdown"})
data = r.json()
