---
name: axonrouter-stt
description: Speech-to-text via AxonRouter /v1/audio/transcriptions using OpenAI Whisper / Groq / Gemini / Deepgram / AssemblyAI / NVIDIA / HuggingFace models. Use when the user wants to transcribe audio, convert speech to text, or get subtitles from audio files.
---

# AxonRouter — Speech-to-Text
...
```bash
curl $AXONROUTER_URL/v1/models/stt | jq '.data[].id'
# Per-model params (language, response_format, prompt, temperature support)
curl "$AXONROUTER_URL/v1/models/info?id=openai/whisper-1"
```
...
`POST $AXONROUTER_URL/v1/audio/transcriptions` (OpenAI Whisper compatible, `multipart/form-data`)
...
```bash
curl -X POST "$AXONROUTER_URL/v1/audio/transcriptions" \
...
const r = await fetch(`${process.env.AXONROUTER_URL}/v1/audio/transcriptions`, {
  method: "POST",
  headers: { "Authorization": `Bearer ${process.env.AXONROUTER_KEY}` },
  body: form,
});
...
with open("audio.mp3", "rb") as f:
    transcript = client.audio.transcriptions.create(model="openai/whisper-1", file=f, language="en")
