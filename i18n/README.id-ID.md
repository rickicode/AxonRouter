<div align="center">
  ...
  **Hubungkan semua tool AI coding (Claude Code, Cursor, Antigravity, Copilot, Codex, Gemini, OpenCode, Cline, OpenClaw...) ke 40+ provider AI dan 100+ model.**
  ...
  [🚀 Mulai Cepat](#-mulai-cepat) • [💡 Fitur](#-fitur-utama) • [📖 Setup](#-panduan-setup) • [🌐 Website](https://axonrouter.com)
...
**AxonRouter menyelesaikan itu semua:**
...
Paket repo ini bersifat privat (`axonrouter-app`), jadi menjalankan dari source/Docker adalah jalur yang diharapkan untuk pengembangan lokal.
...
### 📺 Panduan Setup Lengkap - AxonRouter + Claude Code Gratis

[![AxonRouter + Claude Code Setup](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Tonton tutorial langkah demi langkah:**
- ✅ Install dan setup AxonRouter
...
AxonRouter bekerja mulus dengan semua tool AI coding utama:
...
- Tool CLI mengirim dalam format OpenAI → AxonRouter mengonversi → provider menerima dalam format nativenya
...
> AxonRouter sendiri **tidak menagih apa pun**. Kamu hanya membayar langsung ke provider jika memakai layanan berbayar.
>
...
>
> Anggap saja ini "pelacak penghematan" yang menunjukkan berapa banyak yang kamu hemat lewat model gratis dan routing AxonRouter!
...
### 📊 Tentang Biaya dan Penagihan AxonRouter

**Fakta soal penagihan AxonRouter:**

✅ **Software AxonRouter = gratis selamanya** (open source, tanpa tagihan)
...
❌ **AxonRouter tidak mengirim invoice** atau menagih kartumu
...
- **Provider murah** (GLM, MiniMax): bayar langsung, AxonRouter hanya melakukan routing
- **Provider gratis** (iFlow, Kiro, Qwen): benar-benar gratis selamanya, tanpa biaya tersembunyi
- **AxonRouter**: tidak menagih apa pun
...
Dashboard melacak pemakaian token dan menampilkan **estimasi biaya** seandainya kamu memakai API berbayar secara langsung. Ini **bukan tagihan nyata**, melainkan acuan untuk melihat berapa banyak yang kamu hemat dengan memakai model gratis atau langganan yang sudah ada lewat AxonRouter.
...
<details>
<summary><b>💳 Apakah AxonRouter menagih saya?</b></summary>

**Tidak.** AxonRouter adalah software open source gratis yang berjalan di komputermu sendiri. Tidak ada penagihan sama sekali.

**Kamu membayar ke:**
- ✅ **Provider langganan** (Claude Code $20/bulan, Codex $20-200/bulan) → bayar langsung di website masing-masing
- ✅ **Provider murah** (GLM, MiniMax) → bayar langsung, AxonRouter hanya me-routing request
- ❌ **AxonRouter sendiri** → **tidak menagih apa pun**

AxonRouter adalah proxy/router lokal. Ia tidak menyimpan informasi kartu kredit, tidak bisa mengirim invoice, dan tidak punya sistem penagihan. Sepenuhnya software gratis.
...
AxonRouter hanya me-routing request — tidak ada "jebakan" atau tagihan di kemudian hari. Layanannya memang gratis, dan AxonRouter membuatnya lebih mudah dipakai dengan dukungan fallback.

**Catatan:** beberapa provider langganan (Antigravity, GitHub Copilot) punya masa preview gratis dan bisa jadi berbayar nanti, tetapi hal itu diumumkan secara jelas oleh provider tersebut, bukan oleh AxonRouter.
...
3. **Gunakan provider langganan paling akhir:**
   ...
   - AxonRouter memaksimalkan nilainya lewat pelacakan kuota
...
Smart fallback AxonRouter mencegah tagihan tak terduga:
...
**Kamu yang pegang kendali:** atur batas pengeluaran per provider di dashboard, dan AxonRouter akan mematuhinya.
...
**Tips pro:** pakai Opus untuk tugas kompleks, Sonnet kalau mengutamakan kecepatan. AxonRouter melacak kuota per model!
...
docker run -d \
  ...
  ghcr.io/decolua/axonrouter:latest
```
...
```bash
npm install -g axonrouter
PORT=20128 HOSTNAME=0.0.0.0 BASE_URL=https://your-domain.com axonrouter
```
...
MIT License — lihat [LICENSE](https://github.com/decolua/axonrouter/blob/main/LICENSE) untuk detailnya.

---

<div align="center">

**Kalau AxonRouter membantumu, kasih ⭐ di [GitHub](https://github.com/decolua/axonrouter)!**

[🌐 Website](https://axonrouter.com) • [📦 npm](https://www.npmjs.com/package/axonrouter) • [🐛 Laporkan Bug](https://github.com/decolua/axonrouter/issues)
