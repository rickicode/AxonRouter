<div align="center">
  <img src="../images/axonrouter.png?1" alt="Панель управления AxonRouter" width="800"/>
  
  # AxonRouter - Free AI Router
  ...
  **Бесплатный AI-провайдер для OpenClaw.**
  ...
  [🚀 Быстрый старт](#-quick-start) • [💡 Возможности](#-key-features) • [📖 Установка](#-setup-guide) • [🌐 Сайт](https://axonrouter.com)
</div>
...
Пакет этого репозитория приватный (`axonrouter-app`), поэтому запуск из исходников/Docker — это ожидаемый путь локальной разработки.
...
### 📺 Полное руководство по настройке - AxonRouter + Claude Code БЕСПЛАТНО
  
[![Настройка AxonRouter + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Полное пошаговое руководство:**
- ✅ Установка и настройка AxonRouter
...
AxonRouter бесшовно работает со всеми основными AI-инструментами для кодинга:
...
- Ваш CLI-инструмент отправляет формат OpenAI → AxonRouter транслирует → Провайдер получает родной формат
...
> Сам AxonRouter **никогда ничего не взимает** с вас. Вы платите напрямую провайдерам (если используете платные сервисы).
> 
...
> маршрутизацию через AxonRouter!
...
### 📊 Понимание затрат и оплаты в AxonRouter

**Реальность оплаты AxonRouter:**

✅ **Софт AxonRouter = БЕСПЛАТНО навсегда** (открытый код, никогда не взимает плату)  
...
❌ **AxonRouter никогда не выставляет счёт** и не списывает с вашей карты
...
- **Дешёвые провайдеры** (GLM, MiniMax): Платите им напрямую, AxonRouter только маршрутизирует
- **БЕСПЛАТНЫЕ провайдеры** (iFlow, Kiro, Qwen): Действительно бесплатны навсегда, без скрытых платежей
- **AxonRouter**: Никогда ничего не взимает, никогда
...
Панель отслеживает ваше использование токенов и показывает **оценочные затраты**, как если бы вы напрямую использовали платный API. Это **не реальная оплата** — это справка, показывающая, сколько вы экономите, используя бесплатные модели или существующие подписки через AxonRouter.
...
<details>
<summary><b>💳 Взимает ли с меня плату AxonRouter?</b></summary>

**Нет.** AxonRouter — это бесплатное ПО с открытым кодом, работающее на вашем собственном компьютере. Оно никогда ничего с вас не взимает.

**Вы платите только:**
- ✅ **Провайдерам подписки** (Claude Code $20/мес, Codex $20-200/мес) → Платите им напрямую на их сайте
- ✅ **Дешёвым провайдерам** (GLM, MiniMax) → Платите им напрямую, AxonRouter только маршрутизирует ваши запросы
- ❌ **Самому AxonRouter** → **Никогда ничего не взимает, никогда**

AxonRouter — это локальный прокси/роутер. У него нет вашей кредитной карты, он не может выставлять счета и не имеет платёжной системы. Это полностью бесплатное ПО.
...
AxonRouter только маршрутизирует ваши запросы к ним — никаких «ловушек» или будущих платежей. Это действительно бесплатные сервисы, а AxonRouter облегчает их использование с поддержкой резервирования.

**Примечание:** Некоторые провайдеры подписки (Antigravity, GitHub Copilot) могут иметь бесплатные пробные периоды, которые позже становятся платными, но об этом чётко уведомляют сами провайдеры, а не AxonRouter.
...
3. **Используйте провайдеров подписки в последнюю очередь:**
   ...
   - AxonRouter помогает максимизировать их ценность через отслеживание квоты
...
Умный механизм резервирования AxonRouter предотвращает неожиданные расходы:
...
**Вы контролируете:** Установите лимиты расходов на каждого провайдера в панели, и AxonRouter будет их соблюдать.
...
**Профи-совет:** Используйте Opus для сложных задач, Sonnet для скорости. AxonRouter отслеживает квоту для каждой модели!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [из панели управления axonrouter]
...
> **Примечание:** OpenClaw работает только с локальным AxonRouter. Используйте `127.0.0.1` вместо `localhost`, чтобы избежать проблем с разрешением имён.
...
|----------|---------|-------------|
| `JWT_SECRET` | Автогенерация (`~/.axonrouter/jwt-secret`) | Секрет подписи JWT для cookie аутентификации панели (задайте для общего доступа между инстансами) |
| `INITIAL_PASSWORD` | `123456` | Пароль первого входа при отсутствии сохранённого хеша |
| `DATA_DIR` | `~/.axonrouter` | Расположение основной БД приложения (`db.json`) |
...
| `CLOUD_URL` | `https://axonrouter.com` | Серверный базовый URL эндпоинта облачной синхронизации |
...
| `NEXT_PUBLIC_CLOUD_URL` | `https://axonrouter.com` | Обратно совместимый/публичный облачный URL (приоритет `CLOUD_URL` для серверного рантайма) |
...
- История использования и логи: `~/.axonrouter/usage.json` и `~/.axonrouter/log.txt`, управляется `src/lib/usageDb.js`.
- Опциональные логи запросов/транслятора: `<repo>/logs/...` при `ENABLE_REQUEST_LOGS=true`.
- Хранилище использования следует логике пути `~/.axonrouter` и независимо от `DATA_DIR`.
...
- Убедитесь, что `CLOUD_URL` указывает на ожидаемый облачный эндпоинт (например, `https://axonrouter.com`)
...
- Симптом обычно появляется на публичном облачном эндпоинте (`https://axonrouter.com/v1`) для непотоковых (non-streaming) вызовов.
...
- Локальный рантайм AxonRouter включает резервирование SSE→JSON для непотоковых вызовов, когда upstream возвращает `text/event-stream`.
...
- `tester/security/test-cloud-openai-compatible.sh`
  - Отправляет OpenAI-совместимый запрос напрямую на облачный эндпоинт (`https://axonrouter.com/v1/chat/completions`) с указанной моделью/ключом.
...
- Публичный облачный эндпоинт (`https://axonrouter.com/v1/chat/completions`):
...
- **Сайт**: [axonrouter.com](https://axonrouter.com)
...
Спасибо всем, кто помогает делать AxonRouter лучше!
...
См. [Pull Requests](https://github.com/decolua/axonrouter/pulls) для подробных инструкций.

---

## 🔀 Форки

**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Полнофункциональный TypeScript-форк AxonRouter. Добавляет 36+ провайдеров, авторезервирование на 4 уровнях, мультимодальный API (изображения, embedding, аудио, TTS), circuit breaker, семантическое кеширование, оценку LLM и доработанную панель. 368+ юнит-тестов. Доступен через npm.
