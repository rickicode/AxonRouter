...
import {
  ...
  invalidateProjectId,
  removeConnection,
} from "../../../open-sse/services/projectId";
import {
...
} from "../../../open-sse/services/tokenRefresh";
 ...
 */
type ProviderCredentialsUpdateInput = {
  accessToken?: string;
  refreshToken?: string;
  expiresIn?: number;
...
export async function updateProviderCredentials(connectionId, newCredentials: ProviderCredentialsUpdateInput = {}) {
  try {
    ...
    if (typeof newCredentials.expiresIn === "number") {
    ...
    const result = await updateCurrentProviderConnection(connectionId, updates);
...
export async function checkAndRefreshToken(provider, credentials) {
  ...
  if (creds.expiresAt) {
    ...
    if (remaining < refreshLead) {
      ...
      const newCreds = await runSingleFlight(tokenRefreshFlights, `${provider}:${creds.connectionId || creds.refreshToken || ""}`, () => getAccessToken(provider, creds));
      if (newCreds?.error === "unrecoverable_refresh_error") {
        log.error("TOKEN_REFRESH", "Unrecoverable refresh error, marking connection auth-blocked", {
          provider,
        ...
        if (creds.connectionId) {
          ...
          const { syncUsageStatus } = await import("@/lib/usageStatus");
          await syncUsageStatus(
            { id: creds.connectionId, provider },
            {
              routingStatus: "blocked",
              authState: "expired",
              healthStatus: "unhealthy",
              reasonCode: newCreds.code || "refresh_token_invalid",
              reasonDetail: `Token refresh failed: ${newCreds.code || "unrecoverable"}. Re-authentication required.`,
              lastCheckedAt: new Date().toISOString(),
            },
          );
        }
        return { ...creds, _authBlocked: true };
      }
