...
function withConnectionHeader(response, connectionId) {
  ...
  headers.set("x-axonrouter-connection-id", String(connectionId));
...
export async function handleVideoCreate(request, action) {
  ...
  while (true) {
    ...
    const { shouldFallback } = await markAccountUnavailable(
      credentials.connectionId, result.status, sanitizeSecrets(result.error, refreshedCredentials), provider, model, result.resetsAtMs
    );
...
export async function handleVideoGet(request, requestId, { content = false } = {}) {
  ...
  if (preferredConnectionId) {
    ...
    const queried = new URL(request.url).searchParams.get("provider");
    if (queried && getVideoConfig(queried)) provider = queried;
  }
