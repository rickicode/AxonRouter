...
type QoderConfig = Record<string, any>;
...
export class QoderService {
  private readonly config: QoderConfig = QODER_CONFIG as QoderConfig;
  ...
  async connect() {
    ...
    try {
      ...
      const { port, close }: LocalServerHandle = await startLocalServer((params: CallbackParams) => {
