...
export class KiroService {
   ...
   */
  async refreshToken(refreshToken, providerSpecificData: any = {}) {
   ...
   * @param providerSpecificData - Optional clientId/clientSecret for Builder ID tokens
   */
  async validateImportToken(refreshToken, providerSpecificData?: any) {
    ...
    try {
      // If providerSpecificData not provided, try to register a new client for Builder ID
      let effectiveProviderData = providerSpecificData;
      if (!effectiveProviderData || !effectiveProviderData.clientId) {
        try {
          const registered = await this.registerClient();
          effectiveProviderData = {
          ...
          // Fall through to social auth refresh as last resort
        }
      }

      const result = await this.refreshToken(refreshToken, effectiveProviderData || {});
      return {
        accessToken: result.accessToken,
        refreshToken: result.refreshToken || refreshToken,
        profileArn: result.profileArn,
        expiresIn: result.expiresIn,
        authMethod: effectiveProviderData?.authMethod || "imported",
        providerSpecificData: effectiveProviderData,
      };
    } catch (error) {
