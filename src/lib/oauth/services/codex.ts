...
import { extractCodexIdentity } from "../codexAccount";
...
export class CodexService extends OAuthService {
  ...
  async saveTokens(tokens) {
    ...
    const response = await fetch(`${server}/api/cli/providers/codex`, {
      ...
      body: JSON.stringify({
        ...
        name: identity.name,
        providerSpecificData: identity.providerSpecificData,
      }),
    });
  ...
  async connect() {
    ...
    try {
      ...
      }, fixedPort) as LocalServerHandle;
      ...
      if (!callbackParams) {
        throw new Error("Authentication callback not received");
      }
