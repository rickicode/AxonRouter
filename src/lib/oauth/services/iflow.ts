...
type IFlowConfig = typeof IFLOW_CONFIG;
...
export class IFlowService {
  private readonly config: IFlowConfig = IFLOW_CONFIG;
  ...
  async connect() {
    ...
    try {
      ...
      let callbackParams: OAuthCallbackParams | null = null;
      const { port, close } = await startLocalServer((params: OAuthCallbackParams) => {
