...
const getServerCredentials = () => ({
  server: process.env.AXONROUTER_SERVER_URL || DEFAULT_AXONROUTER_BASE_URL,
  token: process.env.AXONROUTER_CLI_TOKEN || "",
  userId: process.env.AXONROUTER_USER_ID || "cli",
});
...
type AntigravityConfig = typeof ANTIGRAVITY_CONFIG;
...
export class AntigravityService {
  private readonly config: AntigravityConfig = ANTIGRAVITY_CONFIG;
   ...
   */
  async exchangeCode(code: string, redirectUri: string): Promise<AntigravityTokens> {
   ...
   */
  async saveTokens(tokens: AntigravityTokens, userInfo: AntigravityUserInfo, projectId: string) {
  ...
  async connect() {
    ...
    try {
      ...
      const { port, close } = await startLocalServer((params: CallbackParams) => {
        callbackParams = params;
      }) as LocalServerHandle;
      ...
      spinner.fail(`Failed: ${error?.message || "Unknown error"}`);
      throw error;
    }
  }
}
