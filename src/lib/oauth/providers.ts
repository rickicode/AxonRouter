...
const PROVIDERS = {
  ...
  codex: {
    ...
    mapTokens: (tokens) => ({
      ...
      ...(() => {
        ...
        const idCodexIdentity = extractCodexIdentityFromJwt(tokens.id_token);
        const accessCodexIdentity = extractCodexIdentityFromJwt(tokens.access_token);
        ...
        return {
          ...
          providerSpecificData: idCodexIdentity.providerSpecificData || accessCodexIdentity.providerSpecificData || undefined,
        };
      })(),
    }),
  },
  ...
  github: {
    ...
    mapTokens: (tokens, extra) => {
      const githubLogin = extra?.userInfo?.login;
      const githubName = extra?.userInfo?.name;
      const githubEmail = extra?.userInfo?.email;
      const displayName = githubLogin || githubEmail || githubName || undefined;
  ...
  kiro: {
    ...
    requestDeviceCode: async (config, codeChallenge, options: any = {}) => {
  ...
  "amazon-q": {
    ...
    requestDeviceCode: async (config, codeChallenge, options = {}) => {
      return await PROVIDERS.kiro.requestDeviceCode(config, codeChallenge, options);
    },
    pollToken: async (config, deviceCode, codeVerifier, extraData) => {
      return await PROVIDERS.kiro.pollToken(config, deviceCode, codeVerifier, extraData);
    },
    mapTokens: (tokens) => {
      return PROVIDERS.kiro.mapTokens(tokens);
    },
  },
  ...
  gitlab: {
    ...
    flowType: "authorization_code_pkce",
    buildAuthUrl: (config, redirectUri, state, codeChallenge, meta: any = {}) => {
    ...
    exchangeToken: async (config, code, redirectUri, codeVerifier, state, meta: any = {}) => {
      const baseUrl = meta.baseUrl || config.defaultBaseUrl;
      const clientId = meta.clientId || "";
