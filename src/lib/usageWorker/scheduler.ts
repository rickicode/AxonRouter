...
function interruptibleDelay(ms: number, shouldContinue: () => boolean): Promise<void> {
...
export class UsageScheduler {
  ...
  scheduleNext() {
    ...
    this.timerId = setTimeout(() => {
      this.runConnections({ trigger: "timer", mode: "batch" }).catch((error) => {
      ...
      async () => {
        ...
        try {
          ...
          const candidates = allConnections
            ...
            .map((connection) => ({
              connection,
              reason: trigger === "manual_full_refresh" ? "manual_full_refresh" : "scheduled",
            }));
          ...
          await processSequentially(
            ...
            async (entry, index) => {
              ...
              try {
                ...
                if (result.skipped && (result.skipReason === "usage_quota_unavailable" || result.skipReason === "transient_connectivity_error")) {
                  this.logger.warn?.(
