...
function normalizeMorphApiKeyEntry(value, index = 0) {
	...
	return {
		email,
		key,
		status: ["active", "inactive", "cooldown", "exhausted", "unknown"].includes(
			value.status,
		)
			? value.status
			: "unknown",
...
export function stripLegacyMirrorStatusPatch(record = {}) {
	return Object.fromEntries(
		Object.entries(sanitizeConnectionStatusRecord(record || {})).filter(
			([key]) => !LEGACY_MIRROR_STATUS_FIELDS.has(key),
		),
	);
}
...
export function normalizeStoredProviderSpecificData(
	provider,
	...
	const compacted = Object.fromEntries(
		Object.entries(providerSpecificData).filter(
...
function hasCanonicalStatus(connection: any = {}) {
	return CANONICAL_STATUS_KEYS.some(
		(key) => connection?.[key] !== undefined && connection?.[key] !== null,
	);
}
...
function normalizeQuotaExhaustedThresholdPercent(value) {
	if (!Number.isFinite(value))
		return DEFAULT_SETTINGS.quotaExhaustedThresholdPercent;
...
export function normalizeChatRuntimeSettings(settings: any = {}) {
	const source: any =
		settings && typeof settings === "object" && !Array.isArray(settings)
			? settings
			: {};
	const sourceCircuitBreaker =
		source.circuitBreaker &&
		typeof source.circuitBreaker === "object" &&
		!Array.isArray(source.circuitBreaker)
			? source.circuitBreaker
			: {};
	return {
		upstreamTimeoutMs: normalizeOptionalPositiveInteger(
			source.upstreamTimeoutMs,
			DEFAULT_CHAT_RUNTIME_SETTINGS.upstreamTimeoutMs,
		),
		compactUpstreamTimeoutMs: normalizeOptionalPositiveInteger(
			source.compactUpstreamTimeoutMs,
			DEFAULT_CHAT_RUNTIME_SETTINGS.compactUpstreamTimeoutMs,
		),
		codexNonCompactTimeoutMs: normalizePositiveInteger(
			source.codexNonCompactTimeoutMs,
			DEFAULT_CHAT_RUNTIME_SETTINGS.codexNonCompactTimeoutMs,
		),
		codexAgenticTimeoutMs: normalizePositiveInteger(
		...
		maxInflight: normalizePositiveInteger(
			source.maxInflight,
			DEFAULT_CHAT_RUNTIME_SETTINGS.maxInflight,
		),
		providerMaxInflight: normalizePositiveInteger(
			source.providerMaxInflight,
			DEFAULT_CHAT_RUNTIME_SETTINGS.providerMaxInflight,
		),
		accountMaxInflight: normalizePositiveInteger(
			source.accountMaxInflight,
			DEFAULT_CHAT_RUNTIME_SETTINGS.accountMaxInflight,
		),
		observabilityMode: ["full", "sampled", "minimal", "off"].includes(
			source.observabilityMode,
		)
			? source.observabilityMode
			: DEFAULT_CHAT_RUNTIME_SETTINGS.observabilityMode,
		observabilitySampleRate: normalizeUnitInterval(
			source.observabilitySampleRate,
			DEFAULT_CHAT_RUNTIME_SETTINGS.observabilitySampleRate,
		),
		highThroughputSelection: source.highThroughputSelection !== false,
		sseHeartbeatIntervalMs: (() => {
			const raw = normalizeNonNegativeInteger(
				source.sseHeartbeatIntervalMs,
				DEFAULT_CHAT_RUNTIME_SETTINGS.sseHeartbeatIntervalMs,
			);
			return raw === 0 ? 0 : Math.max(5000, raw);
		})(),
		streamReadinessTimeoutMs: normalizePositiveInteger(
			source.streamReadinessTimeoutMs,
			DEFAULT_CHAT_RUNTIME_SETTINGS.streamReadinessTimeoutMs,
		),
		...
		circuitBreaker: {
			...
			failureThreshold: normalizePositiveInteger(
				sourceCircuitBreaker.failureThreshold,
				DEFAULT_CHAT_RUNTIME_SETTINGS.circuitBreaker.failureThreshold,
			),
			resetTimeoutMs: normalizePositiveInteger(
				sourceCircuitBreaker.resetTimeoutMs,
				DEFAULT_CHAT_RUNTIME_SETTINGS.circuitBreaker.resetTimeoutMs,
			),
		},
...
function normalizeProviderRoutingStrategies(value: any = {}) {
	...
	for (const [providerId, configValue] of Object.entries(value)) {
		...
		const strategy = normalizeRoutingStrategy(
			config.strategy || config.fallbackStrategy,
			"",
		);
...
function normalizeRoutingSettings(settings: any = {}) {
	const sourceRouting: any =
		settings?.routing &&
		typeof settings.routing === "object" &&
...
function applyLegacyRoutingAliases(merged) {
	...
	merged.providerStrategies = Object.fromEntries(
		Object.entries(routing.providerStrategies || {}).map(
			([providerId, configValue]) => {
				const config: any = configValue;
				return [
					providerId,
					{
						...
						...(config.strategy === "round-robin"
							? { stickyRoundRobinLimit: config.stickyLimit }
							: {}),
					},
				];
			},
	...
	merged.comboStrategies = Object.fromEntries(
		Object.entries(routing.comboStrategies || {}).map(
			([comboName, configValue]) => {
				const config: any = configValue;
				return [
					comboName,
					{
	...
	merged.stickyDuration =
		routing.sticky?.durationSeconds ||
		DEFAULT_SETTINGS.routing.sticky.durationSeconds;
}
...
export function normalizeMorphSettings(morph: any = {}) {
	...
	const fastApplyModel =
		typeof sourceMorph.fastApplyModel === "string" &&
...
export function mergeSettingsWithDefaults(settings: any = {}) {
	...
	merged.quotaExhaustedThresholdPercent =
		normalizeQuotaExhaustedThresholdPercent(
			sourceSettings?.quotaExhaustedThresholdPercent,
		);

	merged.usageCheck = normalizeUsageCheckSettings(
		sourceSettings?.usageCheck &&
			typeof sourceSettings.usageCheck === "object" &&
			!Array.isArray(sourceSettings.usageCheck)
			? sourceSettings.usageCheck
			: {},
	);
	...
	merged.morphInstructions = {
		...
		...(sourceSettings?.morphInstructions &&
		typeof sourceSettings.morphInstructions === "object" &&
		!Array.isArray(sourceSettings.morphInstructions)
			? {
					enabled: sourceSettings.morphInstructions.enabled !== false,
					mode:
						sourceSettings.morphInstructions.mode === "custom"
							? "custom"
							: "default",
				}
			: {}),
	...
	merged.chatRuntime = normalizeChatRuntimeSettings(
		sourceSettings?.chatRuntime,
	);
	merged.routing = normalizeRoutingSettings(sourceSettings);
	applyLegacyRoutingAliases(merged);

	merged.rateLimitPerKey =
		Number.isFinite(Number(sourceSettings?.rateLimitPerKey)) &&
		Number(sourceSettings?.rateLimitPerKey) >= 0
	...
	const normalized: any = {
		...
		name:
			typeof model.name === "string" && model.name.trim()
				...
				: id,
		source:
			typeof model.source === "string" && model.source.trim()
				? model.source.trim()
				: "imported",
	};
	...
	for (const [providerId, connectionsMap] of Object.entries(input)) {
		...
		for (const [connectionId, models] of Object.entries(connectionsMap)) {
			...
			const normalizedModels = models
				.map(normalizeSyncedAvailableModel)
				.filter(Boolean);
...
export function validateDbImportPayload(payload) {
	...
	if (payload.format !== DB_BACKUP_FORMAT) {
		throw new Error(
			`Invalid database payload format: expected ${DB_BACKUP_FORMAT}`,
		);
	}
	...
	if (
		...
		throw new Error(
			`Invalid database payload schemaVersion: ${payload.schemaVersion}`,
		);
	}
...
export function migrateDbImportPayload(payload) {
	...
	if (schemaVersion < 2) {
		...
		if (Array.isArray(migrated.providerConnections)) {
			migrated.providerConnections = migrated.providerConnections.map(
				(connection) => {
					if (
						!connection ||
					...
					const normalizedProviderSpecificData =
