  ...
  try {
    const mitmPidFile = path.join(
      process.platform === "win32"
        ? path.join(process.env.APPDATA || "", "axonrouter")
...
// Collect PIDs of all AxonRouter-related processes (excluding current)
function collectAppPids(): number[] {
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        const isAppProcess = line.toLowerCase().includes("axonrouter") || line.toLowerCase().includes("next-server");
        if (isAppProcess) {
          const match = line.match(/^"(\d+)"/);
          if (match && match[1] && match[1] !== process.pid.toString()) pids.push(Number(match[1]));
        }
      });
    } catch { /* no processes */ }
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        ...
        if (isAppProcess) {
          ...
          const pid = parts[1];
          if (pid && !isNaN(Number(pid)) && pid !== process.pid.toString()) pids.push(Number(pid));
        }
      });
    } catch { /* no processes */ }
  }
...
export async function killAppProcesses() {
  ...
  pids.forEach(pid => {
    try {
      if (platform === "win32") {
        execSync(`taskkill /F /PID ${pid} 2>nul`, { stdio: "ignore", shell: "cmd.exe", windowsHide: true, timeout: 3000 });
      } else {
