...
import {
	getAutoRoutingTelemetryBreakdown,
	getAutoRoutingTelemetrySummary,
} from "@/lib/routing/autoRoutingTelemetry";
...
function chooseComboForGoal(combos: ComboLike[] = [], goal = "balanced") {
	...
	const exactName = combos.find(
		(combo) => normalizeValue(combo.name) === normalizeValue(goal),
	);
	if (exactName) return exactName;

	if (goal === "balanced") {
		return (
			combos.find((combo) => normalizeValue(combo.name) === "balanced") ||
			combos.find((combo) => normalizeValue(combo.name) === "auto") ||
			[...combos].sort(
				(left, right) => getComboPriority(right) - getComboPriority(left),
			)[0] ||
			null
		);
	}
...
function estimateComboCost(combo: ComboLike = {}) {
	...
	const scored = models.map((step) => {
		...
		if (typeof step === "string") {
			...
			fullModelId =
				providerId && !model.includes("/") ? `${providerId}/${model}` : model;
		} else {
...
function chooseAutoCombo(
	combos: ComboLike[] = [],
	preferredGoal = "balanced",
	telemetryBreakdown: TelemetryBreakdown = {},
) {
	...
	const ranking = [...combos]
		.map((combo) => {
			...
			const comboTelemetry =
				telemetryBreakdown?.byCombo?.[combo.name] ||
				telemetryBreakdown?.byCombo?.[name] ||
				null;
			...
			const goalBoost =
				name === preferredGoal ? 10 : name === "balanced" ? 4 : 0;
			...
			const costPenalty = Number.isFinite(averageCost)
				...
				: 12;
			const score =
				reliabilityScore + sampleBoost + priority + goalBoost - costPenalty;
			return {
				combo,
				score,
				metrics: {
					...
					averageCost: Number.isFinite(averageCost)
						? Number(averageCost.toFixed(3))
						: null,
...
export function isVirtualSystemModel(modelStr: any) {
	return Object.hasOwn(VIRTUAL_SYSTEM_MODELS, normalizeValue(modelStr));
}
...
function normalizeTelemetry(telemetry: any = null): TelemetrySummary {
	...
	const byProvider = Array.isArray(analytics?.byProvider)
	...
	const totalRequests = byProvider.reduce(
...
function resolveAutoGoal(telemetrySummary: TelemetrySummary = {}) {
	...
	if (errorRate >= 0.15 || fallbackRate >= 0.2) {
		return {
			goal: "premium",
			reason:
				"auto escalated to premium because recent error/fallback pressure is elevated",
		};
	}
	...
	if (totalCost >= 50) {
		return {
			goal: "economy",
			reason:
				"auto shifted to economy because recent spend is above the optimizer threshold",
		};
	}

	return {
		goal: "balanced",
		reason: "auto stayed on balanced as the current routing default",
	};
}

export async function resolveVirtualModelExecution({
	modelStr,
	settings = {},
	telemetry = null,
}: VirtualModelExecutionOptions = {}) {
	...
	const telemetryBreakdown: TelemetryBreakdown =
		getAutoRoutingTelemetryBreakdown() as any;
	const autoGoalDecision =
		definition.id === "auto" ? resolveAutoGoal(telemetrySummary) : null;
	const resolvedGoal = autoGoalDecision?.goal || definition.goal;
	const autoComboDecision =
		definition.id === "auto"
	...
	return {
		...
		reason: [
			definition.id === "auto"
				? `${autoGoalDecision?.reason || `auto resolved using ${resolvedGoal}`} and selected combo "${selectedCombo.name}" based on combo telemetry`
				: `${definition.id} virtual model selected built-in combo "${selectedCombo.name}"`,
		],
		telemetrySummary,
		comboRanking:
			autoComboDecision?.ranking?.map((entry) => ({
