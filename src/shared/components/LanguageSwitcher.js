...
export default function LanguageSwitcher({ className = "", isOpen: controlledOpen, onClose, hideTrigger = false }) {
 ...
 disabled={isPending}
 className="flex items-center gap-2 px-3 h-8 rounded-sm text-text-muted hover:text-text-main hover:bg-surface-2"
 title="Language"
 data-i18n-skip="true"
 >
 ...
 {isOpen && createPortal(
 <div className="fixed inset-0 z-50 flex items-center justify-center p-3" data-i18n-skip="true">
 {/* Overlay */}
 <div
 ...
 <div
 ref={modalRef}
 className="relative w-full bg-surface border border-border rounded-sm animate-in fade-in zoom-in-95 max-w-2xl flex flex-col max-h-[80vh]"
 >
 {/* Modal header */}
 ...
 disabled={isPending}
 className={`flex flex-col items-center justify-start gap-1 px-2 py-3 rounded-sm text-xs font-medium w-full ${
 active
