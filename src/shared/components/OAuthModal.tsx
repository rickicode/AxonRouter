...
export default function OAuthModal({ isOpen, provider, providerInfo, onSuccess, onClose, oauthMeta, idcConfig }) {
  ...
  const startOAuthFlow = useCallback(async () => {
    ...
    try {
      ...
      const deviceCodeProviders = ["github", "qwen", "kiro", "amazon-q", "kimi-coding", "kilocode", "codebuddy"];
      if (deviceCodeProviders.includes(provider)) {
        setIsDeviceCode(true);
        setStep("waiting");

        const deviceCodeUrl = new URL(`/api/oauth/${provider}/device-code`, window.location.origin);
        if ((provider === "kiro" || provider === "amazon-q") && idcConfig?.startUrl) {
        ...
        // Pass extraData for Kiro-compatible flows (contains _clientId, _clientSecret)
        const extraData = provider === "kiro" || provider === "amazon-q"
          ? {
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="secondary" onClick={() => copy(authData?.authUrl, "auth_url")}>
                    Copy
                  </Button>
                </div>
              </div>
