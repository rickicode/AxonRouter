...
export default function GitLabAuthModal({ isOpen, providerInfo, onSuccess, onClose }) {
 ...
 with redirect URI{" "}
 <code className="bg-sidebar px-1 rounded-sm text-xs">{getRedirectUri()}</code>
 </p>
 ...
 with scopes: <code className="bg-sidebar px-1 rounded-sm text-xs">api</code>,{" "}
 <code className="bg-sidebar px-1 rounded-sm text-xs">read_user</code>, and{" "}
 <code className="bg-sidebar px-1 rounded-sm text-xs">ai_features</code>.
 </p>
 <Input label="GitLab Base URL" value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} placeholder={GITLAB_COM} />
