...
export const CLI_TOOLS = {
  ...
  copilot: {
    ...
    color: "#1F6FEB",
    description: "GitHub Copilot in VS Code via AxonRouter extension",
    configType: "guide",
    docsUrl: "https://marketplace.visualstudio.com/",
    guideSteps: [
      {
        step: 1,
        title: "Install Extension",
        desc: "In VS Code, open Extensions (Ctrl+Shift+X or Cmd+Shift+X), search for 'AxonRouter for Github Copilot' and click Install.",
      },
      {
        step: 2,
        title: "Configure Server",
        desc: "Press Cmd+Shift+P (or Ctrl+Shift+P), run 'AxonRouter: Configure Server', then enter your Server URL and API Key:",
        value: "{{baseUrl}}",
        copyable: true,
      },
      {
        ...
        desc: "Open Copilot Chat, click the model picker at the bottom → 'Manage Models...' → check the AxonRouter models to use.",
      },
    ],
  },
  opencode: {
...
# cs45 -> cc/claude-sonnet-4-5-20250929`,
  ...
  qwen: {
    ...
    color: "#10B981",
    description: "Alibaba Qwen Code CLI — supports OpenAI, Anthropic & Gemini providers via AxonRouter",
    ...
    notes: [
      ...
      { type: "info", text: "Any model available in AxonRouter can be used — not just Qwen models. Select from Qwen, Claude, Gemini, GPT, and more." },
      { type: "warning", text: "Config path: Linux/macOS ~/.qwen/settings.json • Windows %USERPROFILE%\\.qwen\\settings.json" },
      { type: "error", text: "Qwen OAuth free tier was discontinued on 2026-04-15. Use AxonRouter with alicode/openrouter/anthropic/gemini providers instead." },
    ],
  ...
  jcode: {
    ...
    notes: [
      ...
      {
        type: "info",
        text: "Configure axonrouter as an OpenAI-compatible provider to route all jcode requests through axonrouter's optimization layer."
      },
      {
        type: "warning",
  ...
  "grok-build": {
    ...
    notes: [
      {
        type: "info",
        text: "Grok Build uses ~/.grok/config.toml. AxonRouter writes a [model.axonrouter] custom model and sets it as the default.",
      },
      {
        type: "info",
        text: "After Apply, run grok (or /model axonrouter) to use the routed model. Switch back anytime with /model grok-build.",
      },
      {
        type: "warning",
  ...
  opendesign: {
    ...
    configType: "guide",
    notes: [
      { type: "info", text: "OpenDesign ships as a plugin/skills pack installed into Claude Code, Cursor, OpenAI Codex, Gemini CLI, or OpenCode. It inherits the host agent's model config, so once your host points at AxonRouter, /opendesign design sessions route through AxonRouter automatically — no extra env vars needed." },
    ...
    guideSteps: [
      ...
      { step: 2, title: "No config needed", desc: "OpenDesign runs inside your host agent and uses its model config. If the host already routes through AxonRouter, /opendesign traffic does too." },
