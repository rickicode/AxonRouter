...
function collectBodyRaw(req: any): Promise<Buffer> {
 ...
 */
async function passthrough(req: any, res: any, bodyBuffer: Buffer, onResponse?: (rawBuffer: Buffer, headers: any) => void) {
...
const server = https.createServer(sslOptions, async (req: any, res: any) => {
  try {
    ...
    // Anti-loop: skip requests from AxonRouter
    ...
    const isChat = patterns.some((p: string) => String(req.url || "").includes(p));
    ...
    res.end(JSON.stringify({ error: { message: e?.message || String(e), type: "mitm_error" } }));
  }
});
