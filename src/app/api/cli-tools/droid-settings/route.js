...
// Check if settings has AxonRouter customModels
const hasAxonRouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some((m) => m.id?.startsWith("custom:AxonRouter") || m.id?.startsWith("custom:AxonRouter"));
};
...
// POST - Update AxonRouter customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing AxonRouter configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:AxonRouter"));
...
// DELETE - Remove AxonRouter customModels only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove AxonRouter customModels
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:AxonRouter"));
