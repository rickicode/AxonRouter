...
const hasAxonRouterConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["axonrouter"] || auth["axonrouter"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("axonrouter") || baseUrl.includes("axonrouter");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasAxonRouter: hasAxonRouterConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "AxonRouter", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "AxonRouter settings removed from Kilo Code" });
  } catch (error) {
