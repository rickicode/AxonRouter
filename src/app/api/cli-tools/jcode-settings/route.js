...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-axonrouter.env");
};
...
export async function GET() {
  ...
  const hasAxonRouter = hasAxonRouterConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_AXONROUTER_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile axonrouter",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["axonrouter"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_AXONROUTER_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "axonrouter configuration removed from jcode",
    });
  } catch (error) {
