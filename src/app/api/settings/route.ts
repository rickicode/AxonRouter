...
async function validateMorphApiKeys(
	baseUrl,
	apiKeys = [],
	previousApiKeys = [],
) {
		...
		apiKeys.map(async (entry) => {
			...
			try {
				...
				return {
					...
					lastError:
						validationError?.message || "Failed to validate Morph API key",
				};
			}
		}),
	);
...
export async function GET(request: Request) {
	...
	try {
		...
		const quotaExhaustedThresholdPercent =
			resolveQuotaExhaustedThresholdPercent(
...
export async function PATCH(request: Request) {
	...
	try {
		...
		const needsCurrentSettings =
			body.newPassword ||
			body.morph ||
			body.caveman !== undefined ||
			body.chatRuntime !== undefined ||
			body.resetChatRuntimeDefaults === true;
		const currentSettings: any = needsCurrentSettings
		...
		if (
			...
			if (body.sticky !== undefined || body.stickyDuration !== undefined) {
				nextRouting.sticky = {
					...(baseRouting.sticky || {}),
					...(nextRouting.sticky && typeof nextRouting.sticky === "object"
						? nextRouting.sticky
						: {}),
				};
		...
		if (body.morph !== undefined) {
			const nextMorph = await normalizeCurrentMorphSettings({
				...
				...(body.morph &&
				typeof body.morph === "object" &&
				!Array.isArray(body.morph)
					? body.morph
					: {}),
			});
		...
		if (body.morphInstructions !== undefined) {
			updates.morphInstructions = normalizeMorphInstructionsSettings({
				...
				typeof body.morphInstructions === "object" &&
				!Array.isArray(body.morphInstructions)
					? body.morphInstructions
					: {}),
			});
		}
		...
		if (body.caveman !== undefined) {
			updates.caveman = normalizeCavemanSettings({
				...
				...(body.caveman &&
				typeof body.caveman === "object" &&
				!Array.isArray(body.caveman)
					? body.caveman
					: {}),
			});
		}
					...
					if (value === null || value === undefined) {
						...
						const poolId =
							typeof value === "object" && value !== null
								? (value as any).proxyPoolId
								: value;
						if (typeof poolId === "string" && poolId.trim()) {
							const { getCurrentProxyPoolById } = await import(
							...
							if (!pool) {
								return NextResponse.json(
									{
										error: `Proxy pool "${poolId.trim()}" not found for provider "${key}"`,
									},
									{ status: 400 },
								);
							}
							if (pool.isActive !== true) {
								return NextResponse.json(
									{
										error: `Proxy pool "${poolId.trim()}" is inactive for provider "${key}". Activate it first.`,
									},
									{ status: 400 },
								);
							}
						...
						const groupId =
							typeof value === "object" && value !== null
								? (value as any).proxyGroupId
								: undefined;
						if (typeof groupId === "string" && groupId.trim()) {
							const { getCurrentProxyGroupById } = await import(
								"@/lib/proxyGroupAccess"
							);
							const group = await getCurrentProxyGroupById(groupId.trim());
							if (!group) {
								return NextResponse.json(
									{
										error: `Proxy group "${groupId.trim()}" not found for provider "${key}"`,
									},
									{ status: 400 },
								);
							}
							if (group.isActive !== true) {
								return NextResponse.json(
									{
										error: `Proxy group "${groupId.trim()}" is inactive for provider "${key}". Activate it first.`,
									},
									{ status: 400 },
								);
							}
		...
		if (body.enterprise !== undefined) {
			updates.enterprise = {
				...
				...(body.enterprise &&
				typeof body.enterprise === "object" &&
				!Array.isArray(body.enterprise)
					? body.enterprise
					: {}),
			};
		}
		...
		if (
			...
			updates.chatRuntime = await normalizeCurrentChatRuntimeSettings({
				...baseChatRuntime,
				...(body.chatRuntime &&
				typeof body.chatRuntime === "object" &&
				!Array.isArray(body.chatRuntime)
					? body.chatRuntime
					: {}),
			});
		...
		if (Object.hasOwn(body, "enableRequestLogs")) {
			(globalThis as any).__AXONROUTER_REQUEST_LOGS_ENABLED =
