import { getCurrentProviderConnectionById, updateCurrentProviderConnection } from "../../../../../lib/connectionAccess";
...
const OAUTH_TEST_CONFIG = {
  ...
  github: {
    ...
    authHeader: "Authorization",
    authPrefix: "Bearer ",
    extraHeaders: { "User-Agent": "AxonRouter", "Accept": "application/vnd.github+json" },
  },
  iflow: {
...
function isUsageQuotaExhausted(connection: any = {}) {
  ...
  try {
    ...
    if (res.status === 401 && config.refreshable && !refreshed && connection.refreshToken) {
      ...
      if (tokens) {
        ...
        const retryOpts: any = { method: config.method, headers: retryHeaders };
...
async function fetchWithConnectionProxy(url, options = {}, effectiveProxy = null) {
  // Relay: forward via relay URL
  if (effectiveProxy?.relayUrl) {
    const { proxyAwareFetch } = await import("../../../../../../open-sse/utils/proxyFetch");
...
export async function validateConnectionCredentials(connection) {
  ...
  const effectiveProxy = await resolveConnectionProxyConfig(connection.providerSpecificData || {}, connection.provider);

  if (effectiveProxy.connectionProxyEnabled && effectiveProxy.connectionProxyUrl && !effectiveProxy.relayUrl) {
 ...
 */
export async function testSingleConnection(id, options: any = {}) {
  const { persistStatus = true } = options;
  const connection = await getCurrentProviderConnectionById(id);
  ...
  if (effectiveProxy.connectionProxyEnabled && effectiveProxy.connectionProxyUrl && !effectiveProxy.relayUrl) {
    ...
    if (!proxyResult.ok) {
      ...
      if (persistStatus) {
        await updateCurrentProviderConnection(id, {
  ...
  if (result.refreshed && result.newTokens) {
    const newTokens: any = result.newTokens;
    ...
    if (newTokens.expiresIn) {
      updateData.expiresAt = new Date(Date.now() + newTokens.expiresIn * 1000).toISOString();
    }
  }

  if (persistStatus) {
    await updateCurrentProviderConnection(id, updateData);
  }
