			...
			switch (provider) {
				case "azure": {
					...
					if (!isValid && azureRes.status >= 400) {
						try {
							const errBody = (await azureRes.json()) as {
				...
				case "commandcode": {
					const ccRes = await proxyAwareFetch(
					...
					if (!isValid) {
						...
						let providerMsg =
							"Invalid API key or unsupported Command Code plan";
						try {
							...
							providerMsg =
								parsed?.error?.message || parsed?.message || providerMsg;
						} catch {
				...
				case "freebuff": {
					const sessionRes = await fetch(
						"https://www.codebuff.com/api/v1/freebuff/session",
						{
							...
							headers: {
								...
								"Content-Type": "application/json",
								"User-Agent":
									"ai-sdk/openai-compatible/0.0.96/codebuff-freebuff",
							},
						},
					);
					...
					if (!isValid) {
						error =
							payload?.message || payload?.error || "Invalid Freebuff token";
					}
					break;
				}
				...
				case "nvidia": {
					const endpoints: Record<string, string> = {
						...
						mimo: `${(typeof providerSpecificData?.baseUrl === "string" ? providerSpecificData.baseUrl : "https://api.xiaomimimo.com/v1").replace(/\/+$/, "").replace(/\/chat\/completions$/, "")}/models`,
