...
import { buildMorphManagedConnection, injectMorphManagedProvider } from "./_morphManaged";
...
async function normalizeProxyPoolId(proxyPoolId: unknown): Promise<ProxyPoolResult> {
  ...
  if (proxyPool.isActive !== true) {
    return { error: "Proxy pool is inactive. Activate it first before assigning." };
  }
...
export async function GET(request: Request) {
  ...
  try {
    const rawConnections = (await getCurrentProviderConnections()) as ProviderConnectionLike[];
    const morphManagedConnection = (await buildMorphManagedConnection()) as ProviderConnectionLike;
    const connections = [
      ...(injectMorphManagedProvider(rawConnections) as ProviderConnectionLike[]),
    ...
    for (const [groupKey, groupedConnections] of summaryGroups.entries()) {
      ...
      providerSummaries[providerId][authType] = getProviderConnectionStatusSummary(groupedConnections);
    }
    ...
    const safeConnections = connections.map((connection) => {
      ...
      const maskedApiKey = (typeof rawApiKey === "string" && rawApiKey.length > 6)
        ? `****${rawApiKey.slice(-6)}`
  ...
  try {
    ...
    if (isOpenAICompatibleProvider(provider)) {
      const node = (await getProviderNodeById(provider)) as
        | { prefix?: string; apiType?: string; baseUrl?: string; name?: string }
        | null;
      if (!node) {
      ...
      const node = (await getProviderNodeById(provider)) as
        | { prefix?: string; baseUrl?: string; name?: string }
        | null;
      if (!node) {
    ...
    try {
      ...
      console.warn(`[Providers] Post-create validation failed for ${newConnection.id}: ${message}`);
    }

    const latestConnection = ((await getCurrentProviderConnectionById(newConnection.id as string)) ||
