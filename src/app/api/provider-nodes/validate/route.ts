...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (type === "anthropic-compatible") {
      ...
      const res: any = await fetchWithTimeout(modelsUrl, {
        method: "GET",
        headers: {
          "x-api-key": apiKey,
      ...
      // Fallback: try Anthropic messages endpoint if modelId provided
      if (modelId) {
        const messagesRes: any = await fetchWithTimeout(`${normalizedBase}/messages`, {
          method: "POST",
          headers: {
            "x-api-key": apiKey,
            "anthropic-version": "2023-06-01",
    ...
    if (modelId) {
      const chatRes: any = await fetchWithTimeout(`${baseUrl.replace(/\/$/, "")}/chat/completions`, {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${apiKey}`,
          "Content-Type": "application/json"
