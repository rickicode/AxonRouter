...
import {
  addCurrentCustomModel,
  deleteCurrentCustomModel,
  getCurrentCustomModels,
} from "@/lib/modelCatalogAccess";

export const dynamic = "force-dynamic";

type CustomModelRequestBody = {
...
export async function GET(request: Request) {
  ...
  try {
    const models = await getCurrentCustomModels();
...
export async function POST(request: Request) {
  ...
  try {
    const { providerAlias, id, type, name } = (await request.json()) as CustomModelRequestBody;
    if (!providerAlias || !id) {
      return NextResponse.json({ error: "providerAlias and id required" }, { status: 400 });
    }
    const added = await addCurrentCustomModel({ providerAlias, id, type: type || "llm", name });
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    await deleteCurrentCustomModel({ providerAlias, id, type });
