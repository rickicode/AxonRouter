...
import { VIRTUAL_SYSTEM_MODELS, resolveVirtualModelExecution } from "@/lib/routing/virtualModelResolver";
...
export async function GET() {
  ...
  const spendLeaders = [...providers].sort((a, b) => b.cost - a.cost).slice(0, 3);
  const enterprise = settings?.enterprise || { regionPolicy: "global", complianceMode: "standard", tenantSegregation: false };
  ...
  if (enterprise.tenantSegregation === true) {
    enterpriseInsights.push({
      ...
      summary: "Tenant segregation is enabled. Cross-tenant shared routing assumptions should be reviewed.",
    });
  }
  ...
  return NextResponse.json({
    ...
    context: {
      activeRoutingProfile: (settings as any)?.routing?.profile || (settings as any)?.routingProfile || "balanced",
      lastEvalRunAt: (settings as any)?.evalRuns?.history?.[0]?.timestamp || null,
      lastOptimizerRunAt: settings?.optimizerRuns?.history?.[0]?.timestamp || null,
