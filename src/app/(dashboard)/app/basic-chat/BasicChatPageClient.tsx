...
function loadStoredSessions() {
  try {
    ...
    return Array.isArray(savedSessions)
...
function stripVisibleThinkTags(text) {
  if (typeof text !== "string" || text.length === 0) return "";
  return text.replace(/<think>[\s\S]*?<\/think>/gi, "").replace(/<\/?think>/gi, "");
}

function consumeAssistantVisibleText(text, state) {
  if (typeof text !== "string" || text.length === 0) return "";
  ...
  while (remaining.length > 0) {
    ...
    if (openIndex === -1) {
      visible += remaining;
      return stripVisibleThinkTags(visible);
    }
...
function readAssistantText(chunk, state = { inThink: false }) {
  if (!chunk || typeof chunk !== "object") return "";
  const choice = chunk.choices?.[0];
  const delta = choice?.delta || {};
  const pieces = [delta.content, choice?.message?.content, chunk.output_text, chunk.text]
    .map(textValue)
    .map((text) => consumeAssistantVisibleText(text, state))
    .filter(Boolean);
  return pieces[0] || "";
}
...
export default function BasicChatPageClient() {
  ...
  const [activeSessionId, setActiveSessionId] = useState(() => loadStoredValue(STORAGE_KEYS.activeSessionId, ""));
  const [activeProviderId, setActiveProviderId] = useState(() => loadStoredValue(STORAGE_KEYS.activeProviderId, ""));
  const [activeModelId, setActiveModelId] = useState("");
  const [draft, setDraft] = useState(() => loadStoredValue(STORAGE_KEYS.draft, ""));
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        const [providersRes, providerModelsRes] = await Promise.all([
          fetch("/api/providers", { cache: "no-store" }),
          fetch("/api/provider-models", { cache: "no-store" }),
        ]);
        const providersData = await providersRes.json().catch(() => ({}));
        const providerModelsData = await providerModelsRes.json().catch(() => ({}));
        ...
        const providerModelsByProvider = providerModelsData.models || {};
        ...
        for (const connection of connections) {
          ...
          const aggregateModels = Array.isArray(providerModelsByProvider?.[providerId])
            ? providerModelsByProvider[providerId]
                .map((model) => normalizeLiveModel(model, connection))
                .filter(Boolean)
            : [];
          group.models.push(...aggregateModels);
        }
  ...
  const handleAttachFiles = async (event: any) => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (!reader) {
        ...
        const fallbackText = stripVisibleThinkTags(textValue(data?.choices?.[0]?.message?.content || data?.output_text || data?.error || data?.message || ""));
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            const text = readAssistantText(chunk, thinkState);
            if (!text) continue;
  ...
  return (
    <div className="relative flex-1 flex flex-col h-full min-h-0 min-w-0 bg-[var(--color-editor-panel)] text-[var(--color-text-inverse)] overflow-hidden">
      <div className="relative mx-auto flex flex-1 h-full min-h-0 w-full max-w-4xl flex-col">
        <div className="flex shrink-0 items-center justify-between gap-3 px-4 py-3 lg:px-6">
          <div ref={modelMenuRef} className="relative">
            <button
              ...
              className="flex items-center gap-3 rounded border border-border bg-[var(--color-bg-alt)] px-4 py-3 text-left transition hover:bg-[var(--color-input-bg)]"
            >
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-semibold text-[var(--color-text-inverse)]">{modelLabel}</span>
                  <ChevronDown className="h-[18px] w-[18px] text-[var(--color-text-muted)]" strokeWidth={2} />
                </div>
                <p className="truncate text-xs text-[var(--color-text-muted)]">{modelSubLabel}</p>
              </div>
            </button>

            {modelMenuOpen ? (
              <div className="absolute left-0 top-[calc(100%+10px)] z-30 w-[min(520px,calc(100vw-2rem))] overflow-hidden rounded border border-border bg-[var(--color-surface)]">
                <div className="border-b border-border px-4 py-3">
                  <p className="text-xs uppercase tracking-[0.22em] text-[var(--color-text-subtle)]">Models</p>
                  <p className="text-sm text-[var(--color-text-muted)]">Chỉ lấy từ provider đã connect</p>
                </div>
                <div className="max-h-[60vh] overflow-y-auto p-2 custom-scrollbar">
                  {providerGroups.map((group) => (
                    <div key={group.providerId} className="mb-2 rounded border border-border bg-[var(--color-bg-alt)] p-2">
                        ...
                        {group.models.map((model) => {
                          ...
                          return (
                            <button
                              ...
                              className={`rounded border px-3 py-3 text-left transition ${isActive ? "border-[var(--color-info)]/40 bg-[var(--color-info-soft)]" : "border-border bg-[var(--color-surface)] hover:bg-[var(--color-bg-alt)]"}`}
                            >
                              <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0">
                                  <p className="truncate text-sm font-medium text-[var(--color-text-main)]">{model.name}</p>
                                  <p className="truncate text-[11px] text-[var(--color-text-muted)]">{model.requestModel}</p>
                                </div>
                                {isActive ? <CheckCircle2 size={18} className="text-[var(--color-info)]" strokeWidth={2} /> : null}
                              </div>
                            </button>
                          );
                        })}
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="rounded border border-border bg-[var(--color-bg-alt)] px-4 py-3 text-sm text-[var(--color-text-main)] transition hover:bg-[var(--color-input-bg)]"
            >
              History
            </button>
            <Button variant="ghost" size="sm" onClick={handleDeleteCurrentChat} disabled={!activeSessionId || sessions.length === 0}>
        ...
        {historyOpen ? (
          <div ref={historyMenuRef} className="absolute right-4 top-[72px] z-20 w-[min(360px,calc(100vw-2rem))] rounded border border-border bg-[var(--color-surface)] p-2 lg:right-6">
            <div className="px-3 py-2">
              <p className="text-xs uppercase tracking-[0.22em] text-[var(--color-text-subtle)]">Recent chats</p>
            </div>
            <div className="max-h-[48vh] space-y-2 overflow-y-auto p-1 custom-scrollbar">
              {sessionItems.length === 0 ? (
                <div className="rounded border border-dashed border-border bg-[var(--color-bg-alt)] p-4 text-sm text-[var(--color-text-muted)]">
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded border px-3 py-3 text-left transition ${isActive ? "border-[var(--color-info)]/40 bg-[var(--color-info-soft)]" : "border-border bg-[var(--color-surface)] hover:bg-[var(--color-bg-alt)]"}`}
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-medium text-[var(--color-text-main)]">{session.title}</p>
                        <p className="mt-1 truncate text-xs text-[var(--color-text-muted)]">{textValue(latestMessage?.content) || "Empty chat"}</p>
                      </div>
                      <span className="text-[10px] text-[var(--color-text-subtle)] shrink-0">{formatRelativeTime(session.updatedAt)}</span>
                    </div>
                  </button>
                );
              })}
        ...
        {loadError ? (
          <div className="mt-4 rounded border border-[var(--color-danger)]/20 bg-[var(--color-danger-soft)] px-4 py-3 text-[var(--color-danger)]">
        ...
        <div className="flex flex-1 flex-col min-h-0">
            ...
            {currentMessages.length === 0 ? (
              <div className="flex min-h-[50vh] items-center justify-center px-4 text-center">
                <div className="max-w-xl space-y-4">
                  <div className="mx-auto flex size-16 items-center justify-center rounded border border-border bg-[var(--color-bg-alt)] text-[var(--color-text-main)]">
                    <MessageSquareText size={30} strokeWidth={2} />
                  </div>
                  <div className="space-y-2">
                    <h2 className="text-2xl font-semibold text-[var(--color-text-inverse)]">Start a conversation</h2>
              ...
              {currentMessages.map((message) => {
                ...
                return (
                  <div key={message.id} className={`flex w-full ${isUser ? "justify-end" : "justify-start"} mb-6`}>
                    <div className={`max-w-[min(88%,42rem)] ${isUser ? "rounded border border-border bg-[var(--color-bg-alt)] px-5 py-3.5 text-[var(--color-text-inverse)]" : "text-[var(--color-text-inverse)]"}`}>
                      ...
                      {message.attachments?.length ? (
                        <div className="mb-3 grid grid-cols-2 gap-2 sm:grid-cols-3 mt-2">
                          {message.attachments.map((attachment) => (
                            <a key={attachment.id} href={attachment.dataUrl} target="_blank" rel="noreferrer" className="overflow-hidden rounded border border-border bg-[var(--color-bg-alt)]">
                              <Image src={attachment.dataUrl} alt={attachment.name} width={224} height={112} unoptimized className="h-28 w-full object-cover" />
                            </a>
                          ))}
                        </div>
                      ) : null}
          ...
          <div className="shrink-0 pt-2">
            {attachments.length > 0 ? (
              <div className="mx-auto mb-3 flex w-full max-w-3xl flex-wrap gap-2 px-4">
                {attachments.map((attachment) => (
                  <div key={attachment.id} className="flex items-center gap-2 rounded-full border border-border bg-[var(--color-bg-alt)] px-3 py-2">
                    <span className="text-xs text-[var(--color-text-main)] max-w-[12rem] truncate">{attachment.name}</span>
                    <button type="button" onClick={() => removeAttachment(attachment.id)} className="text-[var(--color-text-muted)] hover:text-[var(--color-text-main)] cursor-pointer" aria-label="Remove attachment">
            ...
            <div className="mx-auto w-full max-w-3xl px-4 pb-2">
              <div className="rounded border border-border bg-[var(--color-surface)] px-3 pt-3 pb-2">
                <textarea
                  value={draft}
                  ...
                  rows={1}
                  className="w-full resize-none bg-transparent px-2 text-[15px] leading-6 text-[var(--color-text-main)] outline-none placeholder:text-[var(--color-text-subtle)] custom-scrollbar max-h-[25vh] overflow-y-auto"
                />

                <div className="mt-2 flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={() => fileInputRef.current?.click()} disabled={!activeModel || loadingData} className="p-2 text-[var(--color-text-muted)] hover:text-[var(--color-text-main)] transition rounded-full hover:bg-[var(--color-bg-alt)] cursor-pointer">
                      <Paperclip size={20} strokeWidth={2} />
                    </button>
                    <input ref={fileInputRef} type="file" accept="image/*" multiple className="hidden" onChange={handleAttachFiles} />
                    <span className="text-xs font-medium text-[var(--color-text-subtle)] truncate max-w-[120px]">{activeModel ? activeModel.name : "No model"}</span>
                  </div>

                  <div className="flex items-center gap-2">
                    {isSending ? (
                      <button type="button" onClick={handleStop} className="p-2 text-[var(--color-text-main)] bg-[var(--color-bg-alt)] hover:bg-[var(--color-input-bg)] transition rounded-full h-8 w-8 flex items-center justify-center cursor-pointer">
                        <Square size={16} strokeWidth={2} />
                      </button>
                    ) : null}
                    <button onClick={sendMessage} disabled={!canSend} className={`h-8 w-8 rounded-full flex items-center justify-center transition ${canSend ? 'bg-[var(--color-primary)] text-white hover:opacity-90 cursor-pointer' : 'bg-[var(--color-bg-alt)] text-[var(--color-text-subtle)] cursor-not-allowed'}`}>
          ...
          <p className="mx-auto mt-2 max-w-3xl px-4 pb-4 text-center text-[11px] text-[var(--color-text-subtle)]">
