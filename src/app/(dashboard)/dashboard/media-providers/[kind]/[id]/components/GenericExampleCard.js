...
export function GenericExampleCard({ providerId, kind }) {
 ...
 alt="Reference"
 className="max-h-40 rounded-sm border border-border object-contain bg-sidebar"
...
<Icon name={running ? "progress_activity" : "check_circle"} size={18} className="text-primary" style={running ? { animation: "spin 1s linear infinite" } : undefined} />
 ...
 <div>
 <span className="text-xs font-medium text-text-muted">Partial preview</span>
 <img
 src={`data:image/png;base64,${partialImage.b64_json}`}
 alt="Partial"
 className="max-w-full rounded-sm border border-border mt-1.5 opacity-80"
 loading="lazy"
 decoding="async"
 />
 </div>
