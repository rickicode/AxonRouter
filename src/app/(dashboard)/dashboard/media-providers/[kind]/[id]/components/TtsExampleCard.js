...
export function TtsExampleCard({ providerId }) {
 ...
 .catch(() => {});
 fetch(`/api/providers?provider=${encodeURIComponent(providerId)}&isActive=true&fields=summary`, { cache: "no-store" })
 .then((r) => r.json())
 ...
 <button
 onClick={openModal}
 className="w-full min-w-0 flex-1 px-3 py-2 text-sm border border-border rounded-sm bg-surface font-mono truncate text-left hover:border-primary/30"
 >
 {selectedLang
 ...
 <span className="ml-1.5 px-1 py-1 text-[11px] font-medium rounded-sm bg-success/10 text-success border border-success/30">Free</span>
 )}
 {v.free_users_allowed === false && (
 <span className="ml-1.5 px-1 py-1 text-[11px] font-medium rounded-sm bg-warning/10 text-warning border border-warning/30">Paid</span>
 )}
 </button>
 ))}
 </div>
 ...
 className="w-full px-3 py-2 pr-7 text-sm border border-border rounded-sm bg-surface focus:outline-none focus:border-primary font-mono"
 />
 {voiceId && (
 <button
 type="button"
 ...
 className="w-full px-3 py-2 pr-7 text-sm border border-border rounded-sm bg-surface focus:outline-none focus:border-primary"
 />
 {input && (
 <button
 type="button"
 ...
 rows={2}
 className="w-full px-3 py-2 pr-7 text-sm border border-border rounded-sm bg-surface focus:outline-none focus:border-primary resize-none"
 />
 {style && (
 <button
 type="button"
...
<Icon name={copiedCurl ? "check" : "content_copy"} size={18} />
 ...
 >
 <div
 className="border border-border rounded-sm w-full max-w-md mx-4 flex flex-col max-h-[80vh]"
 ...
 {/* Search */}
 <div className="px-3 h-8 border-b border-border shrink-0">
 <input
 autoFocus
 value={modalSearch}
 ...
 className={`flex items-center justify-between w-full px-3 h-8 rounded-sm text-left hover:bg-surface-2 ${
