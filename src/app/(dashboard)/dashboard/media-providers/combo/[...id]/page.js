...
export default function ComboDetailPage() {
 ...
 ? `curl -X POST http://localhost:3777${examplePath} \\\n -H "Content-Type: application/json" \\\n -H "Authorization: Bearer ${apiKey || "YOUR_KEY"}" \\\n -d '${JSON.stringify(exampleBody)}'`
 : "";
...
<Icon name={badge.icon} size={18} />
 ...
 <code className="text-sm font-semibold font-mono">{combo.name}</code>
 </div>
 </div>
 <Button variant="outline" icon="delete" onClick={handleDelete} className="text-danger border-danger/30 hover:bg-danger/10">
 Delete
 </Button>
 </div>
 ...
 <p className="text-[11px] text-text-muted mt-0.5">Only letters, numbers, -, _, . and /</p>
 </div>
 ...
 return (
 <div key={`${entry}-${idx}`} className="flex items-center gap-3 p-3 rounded-sm bg-surface-2">
 ...
 {model && <code className="text-[11px] text-text-muted font-mono truncate block">{model}</code>}
 </div>
 ...
 <Card>
 <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-3">
 <h2 className="text-sm font-semibold">Test Example</h2>
 <Button size="sm" icon="play_arrow" onClick={handleTest} disabled={testing || providers.length === 0}>
 {testing ? "Running..." : "Run"}
 </Button>
 </div>
 <pre className="text-xs font-mono p-3 rounded-sm overflow-x-auto whitespace-pre-wrap break-all border border-border bg-surface text-text-main">
 {curlExample}
 </pre>
 {testError && (
 ...
 <img src={testResult.imageUrl} alt="Generated" className="max-w-full rounded-sm border border-border" loading="lazy" decoding="async" />
 </div>
 )}
 {testResult.audioUrl && (
 <div>
 ...
 {testResult.json && (
 <pre className="text-xs font-mono p-3 rounded-sm overflow-auto max-h-[300px] whitespace-pre-wrap break-all border border-border bg-surface text-text-main">
 {testResult.json}
 </pre>
 )}
 </div>
 ...
 {/* Usage Logs Card */}
 <Card>
 <h2 className="text-sm font-semibold mb-3">Usage Logs</h2>
 {logs.length === 0 ? (
 <p className="text-xs text-text-muted italic">No usage yet.</p>
 ) : (
 <pre className="text-[11px] font-mono p-3 rounded-sm overflow-auto max-h-[400px] whitespace-pre-wrap border border-border bg-surface text-xs text-text-main">
 {logs.join("\n")}
 </pre>
 )}
 </Card>
