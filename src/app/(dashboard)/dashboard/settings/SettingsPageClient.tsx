...
export default function SettingsPageClient() {
  ...
  const [usageCheckSettings, setUsageCheckSettings] = useState({ enabled: true, intervalMinutes: 60 });
  const [savingUsageCheck, setSavingUsageCheck] = useState(false);
  const [usageCheckFeedback, setUsageCheckFeedback] = useState({ type: "", message: "" });
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      setUsageCheckSettings({
        ...
        intervalMinutes: Number.isFinite(uc.intervalMinutes) && uc.intervalMinutes > 0 ? Math.max(1, Math.round(uc.intervalMinutes)) : 60,
      });
    });
  }, [settingsQuery.data]);
  ...
  async function handleSaveUsageCheck() {
    ...
    try {
      ...
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.error || "Failed to save usage check settings");
      // Reload scheduler to pick up new interval immediately
      await fetch("/api/usage-worker/status", { method: "POST", cache: "no-store" }).catch(() => {});
      setUsageCheckFeedback({ type: "success", message: "Usage check settings saved. Scheduler restarted with new interval." });
      inv.settings();
    } catch (error: any) {
      setUsageCheckFeedback({ type: "error", message: error?.message || "Failed to save usage check settings" });
    } finally {
  ...
  return (
    <div className="flex flex-col gap-4">
          ...
          <Card><CardHeader><div><CardTitle>Usage Check Scheduler</CardTitle></div></CardHeader><CardContent>
            <div className="flex flex-col gap-4">
              {usageCheckFeedback.message ? (
                <Alert
                  variant={usageCheckFeedback.type === "error" ? "destructive" : "default"}
                  className="rounded-[4px]"
                >
                  <AlertDescription>{usageCheckFeedback.message}</AlertDescription>
                </Alert>
              ) : null}
              ...
              <div className="grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]">
                  ...
                  <div className="flex flex-col gap-4">
                    <label className="flex gap-3 rounded-[4px] border border-[var(--color-border)] bg-[var(--color-bg)] p-4 text-sm text-[var(--color-text-main)]">
                      <Switch
                        ...
                        onToggle={(checked) => setUsageCheckSettings((current) => ({ ...current, enabled: checked }))}
                        disabled={savingUsageCheck}
                      />
                      <span className="flex flex-col gap-1">
                        <span className="block font-medium">Enable automatic usage check</span>
                        <span className="text-[var(--color-text-muted)]">AxonRouter will periodically check provider quota usage for all active OAuth connections.</span>
                      </span>
                    </label>
                      ...
                      <Input
                        ...
                        onChange={(event) => {
                          ...
                          setUsageCheckSettings((current) => ({
                            ...current,
                            intervalMinutes: Number.isFinite(value) && value > 0 ? Math.max(60, Math.round(value)) : 60,
                          }));
                        }}
                        disabled={savingUsageCheck}
                      />
                      <FieldDescription>
                        How often the scheduler checks usage for all connections. Default: 60 minutes. Minimum: 60 minutes.
                      </FieldDescription>
                    </Field>
                  </div>
                </div>
                ...
                <div className="rounded-[4px] border border-[var(--color-border)] bg-[var(--color-bg-alt)]/78 p-4 text-sm leading-6 text-[var(--color-text-muted)]">
                  <p><span className="font-medium text-[var(--color-text-main)]">Behavior:</span> Connections with prior failures are skipped (backoff). Accounts that are disabled or have invalid auth are permanently skipped.</p>
                  <p className="mt-2"><span className="font-medium text-[var(--color-text-main)]">Backoff levels:</span> 1m → 5m → 15m → 30m → 1h → 2h → 4h (escalating with consecutive failures).</p>
                  <p className="mt-2"><span className="font-medium text-[var(--color-text-main)]">Force re-check:</span> Use the yellow refresh icon on blocked accounts in the Usage page to reset backoff and immediately re-check.</p>
                </div>
              </div>

              <div className="flex flex-wrap gap-2">
                <Button onClick={handleSaveUsageCheck} disabled={savingUsageCheck}>
