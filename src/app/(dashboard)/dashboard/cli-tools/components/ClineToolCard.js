...
export default function ClineToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
 ...
 const [showManualConfigModal, setShowManualConfigModal] = useState(false);
  ...
  useEffect(() => {
  if (!status?.settings?.openAiModelId) return;
  let cancelled = false;
  queueMicrotask(() => {
  if (cancelled) return;
 ...
 <Image src="/providers/cline.png" alt={tool.name} width={32} height={32} className="size-8 object-contain rounded-sm" sizes="32px" onError={(e) => { e.target.style.display = "none"; }} loading="lazy" decoding="async" />
 </div>
