...
export default function AddApiKeyModal({ isOpen, provider, providerName, isCompatible, isAnthropic, authType, authHint, website, proxyPools, error, existingNames, onSave, onBulkDone, onClose }) {
 ...
 setValidating(false);
  ...
  const handleSubmit = async () => {
    ...
    // Custom provider: defaultModel optional (models fetched after connection created)
 ...
 setValidating(false);
  ...
  await onSave({
    ...
    // Master: defaultModel optional for compatible providers (models fetched after connection created)
 ...
 <textarea
 className="w-full rounded-sm border border-border bg-surface p-3 text-sm font-mono resize-y min-h-[140px] focus:outline-none"
 ...
 {isCloudflareAi && (
 <div className="bg-sidebar/50 p-3 rounded-sm border border-primary/30">
