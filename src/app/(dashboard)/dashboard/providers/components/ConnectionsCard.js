...
function ConnectionRow({ connection, proxyPools, isOAuth, isFirst, isLast, onMoveUp, onMoveDown, onToggleActive, onUpdateProxy, onEdit, onDelete }) {
 ...
 return (
 <div className={`group flex flex-col gap-3 p-3 rounded-sm sm:flex-row sm:items-center sm:justify-between hover:bg-surface-2 ${connection.isActive === false ? "opacity-60" : ""} ${showProxyDropdown ? "relative z-30" : ""}`}>
 ...
 <span
 className="text-xs text-warning truncate max-w-[320px]"
 ...
 {maskedProxyUrl && <code className="text-[11px] font-mono bg-surface px-1 py-1 rounded-sm text-text-muted">{maskedProxyUrl}</code>}
...
function AddApiKeyModal({ isOpen, provider, providerName, proxyPools, onSave, onClose }) {
 ...
 <div>
 <label className="text-xs text-text-muted mb-1 block font-medium">Name</label>
 <input className="h-8 w-full border border-border bg-surface px-2 text-sm text-text-main outline-none focus:border-primary" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="Production Key" />
 </div>
 <div className="flex gap-2">
 <div className="flex-1">
 <label className="text-xs text-text-muted mb-1 block font-medium">API Key</label>
 <input type="password" className="h-8 w-full border border-border bg-surface px-2 text-sm text-text-main outline-none focus:border-primary" value={formData.apiKey} onChange={(e) => setFormData({ ...formData, apiKey: e.target.value })} />
 </div>
 <div className="pt-3">
 ...
 <div>
 <label className="text-xs text-text-muted mb-1 block font-medium">Priority</label>
 <input type="number" className="h-8 w-full border border-border bg-surface px-2 text-sm text-text-main outline-none focus:border-primary" value={formData.priority} onChange={(e) => setFormData({ ...formData, priority: Number.parseInt(e.target.value) || 1 })} />
 </div>
...
export default function ConnectionsCard({ providerId, isOAuth }) {
 ...
 if (loading) return <Card><div className="h-20 animate-pulse bg-surface-2 rounded-sm" /></Card>;
 ...
 className="w-16 px-2 py-1 text-xs border border-border rounded-sm bg-surface focus:outline-none focus:border-primary"
 />
 </div>
 )}
 </div>
 ...
 of <span className="font-medium text-text-main">{pagination.total}</span> connections (Page {pagination.page} of {pagination.totalPages})
 </span>
